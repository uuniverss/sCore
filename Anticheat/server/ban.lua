...
AddEventHandler('Initiate:BanSql', function(license, id, reason, name, time, perma)
    ...
    if name ~= nil then
        DropPlayer(id, "Vous avez été banni de Ever Life\nRaison : "..reason.."\nAuteur : "..name.."\n")
    end
    ...
    TriggerClientEvent("esx:showNotification",source, "Le joueur à bien été ~g~ban~s~~n~License : "..license)
end)
    ...
    MySQL.Async.fetchAll('SELECT License FROM players_ban WHERE License = @License',
    {
        ...
        if data[1] then
            ...
            if source ~= 0 then
                ...
                print("Le joueur à bien été ~g~ban~s~~n~License : "..license)
            end
        else
                        ...
                        ["columns"] = {
                            {
                                ...
                                ["items"] = {
                                    {
                                        ["type"] = "Image",
                                        ["url"] = "https://cdn.discordapp.com/attachments/1097923643931107479/1132302036692635788/512x512.png",
                                        ["size"] = "Small"
                                    }
                                },
                                ["width"] = "auto"
        ...
        if tonumber(args[1]) then
            MySQL.Async.fetchAll('SELECT License FROM players_ban WHERE ID = @ID',
            {
                ...
                if data[1] then
                    ...
                    ReloadBans()
                    TriggerClientEvent('esx:showNotification', source, "La license à bien été ~g~unban~s~.") 
                else
...
function kickPlayer(source, target, reason)
    ...
    TriggerClientEvent("esx:showNotification", source, "Vous avez ~r~kick~s~ le joueur ~g~"..targetName.."~s~ !")
end
