...
local accesCreateLab = {
    ["license:4968d9411554d28590a0b51925f7b7f1a4f7aae3"] = true, --DEN--
}
...
RegisterServerEvent("sLaboratoire:createLab")
AddEventHandler("sLaboratoire:createLab", function(type, name, price, pos)
    if source then
        ...
        MySQL.Async.execute('INSERT INTO lab_list (type, name, price, pos, time) VALUES (@type, @name, @price, @pos, @time)', {
            ...
            TriggerClientEvent("esx:showNotification", xPlayer.source, "Vous venez de créer un ~g~"..name.."~s~ à "..price.."~g~$~s~ !")
            TriggerClientEvent("sLaboratoire:refreshLab", -1)
       end) 
    end
end)

RegisterServerEvent("sLaboratoire:deleteLab")
        ...
        MySQL.Async.fetchAll('SELECT * FROM lab_list WHERE `type` = @type AND `id` = @id', {
            ...
            if result[1] then
                MySQL.Async.execute("DELETE FROM lab_list WHERE `type` = @type AND `id` = @id", {
                    ...
                    TriggerClientEvent("esx:showNotification", xPlayer.source, "Le laboratoire n°~g~"..id.."~s~ viens d'être ~r~supprimé~s~ !")
                    TriggerClientEvent("sLaboratoire:refreshLab", -1)
                end)
            else
                TriggerClientEvent("esx:showNotification", xPlayer.source, "~r~Le laboratoire n°~g~"..id.."~s~ n'existe pas !")
            end
       end)
    end
end)

RegisterServerEvent("sLaboratoire:buyLab")
    ...
    if source then
        ...
        if xPlayer.getAccount("cash").money >= table.price then
            MySQL.Async.execute('UPDATE lab_list SET owner = @owner, ownerName = @ownerName, time = @time WHERE id = @id', {
                ...
                TriggerClientEvent("esx:showNotification", xPlayer.source, "Vous venez de acheter un ~g~"..table.name.."~s~ pour "..table.price.."~g~$~s~")
...
RegisterServerEvent("sLaboratoire:removeOwner")
AddEventHandler("sLaboratoire:removeOwner", function(table, owner, ownerName)
    if source then
        ...
        MySQL.Async.execute('UPDATE lab_list SET owner = @owner, ownerName = @ownerName WHERE id = @id', {
            ...
            TriggerClientEvent("esx:showNotification", xPlayer.source, "Vous venez de dissocier le ~g~"..table.name.."~s~ des ~g~"..ownerName.."~s~ !")
            TriggerClientEvent("sLaboratoire:returnBuyLab", xPlayer.source)
            TriggerClientEvent("sLaboratoire:refreshLab", -1)               
        end)
...
RegisterServerEvent("sLaboratoire:Enter")
AddEventHandler("sLaboratoire:Enter", function(pos, table)
    if source then
        ...
        SetPlayerRoutingBucket(source, table.id)
        exports.es_extended:onSavedPlayer(false, xPlayer.identifier)
        TriggerClientEvent("sLaboratoire:returnEnter", source, table)
   end
end)

RegisterServerEvent("sLaboratoire:Visite")
...
ESX.RegisterServerCallback('sLaboratoire:SendPlayersAttack', function(source, cb, tableplayers, tablelablist)
    ...
    if attackers >= 6 and defense >= 6 then
        cb(true)
        for k,v in pairs(tableplayers) do
            local player = ESX.GetPlayerFromId(tonumber(v.playersid))
    ...
    if source then
        ...
        for i = 1, #xPlayers do
            ...
            if xPlayers.job2.name == xPlayer.job2.name then
                TriggerClientEvent("esx:showNotification", xPlayers.source, "Vous êtes en train ~r~d'attaquer~s~ le laboratoire appartenant au ~g~"..ownerName.."~s~.")
                TriggerClientEvent("esx:showNotification", xPlayer.source, "Vous êtes en train ~r~d'attaquer~s~ le laboratoire appartenant au ~g~"..ownerName.."~s~.")
            end
            if xPlayers.job2.name == owner then
                TriggerClientEvent("esx:showNotification", xPlayers.source, "Votre laboratoire situé à ~g~"..zone.."~s~ est actuellement en train de se faire ~r~attaquer~s~ aller le défendre au plus vite.")
            end
        end
    end
end)
...
AddEventHandler("sLaboratoire:updateAttack", function(id, name, owner, ownerName)
    if source then
        ...
        for i = 1, #xPlayers do
            ...
            local xPlayers = ESX.GetPlayerFromId(xPlayers[i])
            if xPlayers.job2.name == xPlayer.job2.name then
                TriggerClientEvent("esx:showNotification", xPlayers, "Vous venez de capturer le laboratoire de ~g~"..name.."~s~.")
                TriggerClientEvent("esx:showNotification", source, "Vous venez de capturer le laboratoire de ~g~"..name.."~s~.")
            end
        end
        TriggerClientEvent("sLaboratoire:refreshLab", -1)     
    end
