        ...
        Citizen.CreateThread(function()
            while openedCreatedMenu do
                ...
                RageUI.IsVisible(createdMenu, false, false, true, function()
                    RageUI.Button("Liste des laboratoires", nil, {RightLabel = "~g~Voir~s~ →"}, true, function(_, _, Select)
                        if (Select) then
                    ...
                    RageUI.Separator("")
                    RageUI.Separator("↓ Création d'un ~g~laboratoire~s~ ↓")
                    if not createLab.selectLabo then
                        ...
                        RageUI.Separator("Laboratoire choisi ~g~"..createLab.selectLaboLabel)
                    end
                        ...
                        if #resultLab == 0 then
                            ...
                            for k,v in pairs(resultLab) do
                                if v.owner == nil then
                                    ...
                                    ownerName = "au ~g~"..v.ownerName.."~s~"
                                end
                                RageUI.Button(v.name.." - "..v.price.."~g~$~s~", "Appartenant "..ownerName..", laboratoire n°"..v.id, {RightLabel = "~g~Voir~s~ →"}, true, function(_, _, Select)
                                    if (Select) then
                ...
                RageUI.IsVisible(createdViewMenu, false, false, true, function()
                    RageUI.Separator("Laboratoire n°~g~"..selectLab.id)
                    RageUI.Separator("Type de laboratoire ~g~"..selectLab.type)
                    ...
                    RageUI.Button("Se téléporter", "~r~Attention~s~ : Pas de retour possible !", {RightLabel = "~g~Oui~s~ →"}, true, function(_, _, Select)
                        if (Select) then 
                            ...
                            ESX.ShowNotification("Vous vous êtes bien ~g~téléporté~s~ au ~g~"..selectLab.name.."~s~ n°~g~"..selectLab.id.."~s~ !")
                            RageUI.CloseAll()
                            openedCreatedMenu = false
                        end
                    end)
                    if selectLab.owner ~= nil then
                        RageUI.Button("Dissocier le laboratoire des ~g~"..selectLab.ownerName, "~r~Attention~s~ : Pas de retour possible !", {RightLabel = "~g~Oui~s~ →"}, true, function(_, _, Select)
                            if (Select) then 
