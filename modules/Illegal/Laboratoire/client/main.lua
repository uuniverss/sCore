...
Labo.Zone = {
	...
	{pos = vector3(978.17633056641,-146.64123535156,-48.9), labocolor = "~g~", itemwin = "meth", itemcountwin = 10, itemrequired = "meth_lode", itemcountrequired = 5, itemrequired2 = "meth_phosphore", itemcountrequired2 = 5, progress = {duration = 3200, name = "Traitement de votre meth", r = 0, g = 141, b = 223}, notif = "Appuyez sur ~INPUT_CONTEXT~ pour ~g~traiter votre meth~s~.", animID = 5, size = 2.5},
	{pos = vector3(985.88610839844,-141.49186706543,-48.999626159668), labocolor = "~b-~", itemwin = "meth_pooch", itemcountwin = 10, itemrequired = "meth", itemcountrequired = 10, progress = {duration = 3200, name = "Conditionnement de votre meth", r = 0, g = 141, b = 223}, notif = "Appuyez sur ~INPUT_CONTEXT~ pour ~g~conditionner votre meth~s~.", animID = 6, size = 2.5},

	{pos = vector3(1093.18, -3194.925, -39.60), labocolor = "~g~", itemwin = "coke", itemcountwin = 10, itemrequired = "coca_feuille", itemcountrequired = 5, itemrequired2 = "coca_acide", itemcountrequired2 = 5, progress = {duration = 3200, name = "Traitement de votre coke", r = 248, g = 226, b = 0}, notif = "Appuyez sur ~INPUT_CONTEXT~ pour ~g~traiter votre coke~s~.", animID = 3, size = 2.5},
	{pos = vector3(1101.245, -3198.82, -39.60), labocolor = "~g~", itemwin = "coke_pooch", itemcountwin = 10, itemrequired = "coke", itemcountrequired = 10, progress = {duration = 3200, name = "Conditionnement de votre coke", r = 248, g = 226, b = 0}, notif = "Appuyez sur ~INPUT_CONTEXT~ pour ~g~conditionner votre coke~s~.", animID = 4, size = 2.5},
}
                ...
                if IsControlJustPressed(0, 51) then 
                    if v.itemrequired then
                            ...
                            if count >= v.itemcountrequired then 
                                if v.itemrequired2 then 
                                    ESX.TriggerServerCallback("sDrugs:getItemAmount", function(count)
                                        if count >= v.itemcountrequired2 then
                                            ...
                                            if not IsPedSittingInAnyVehicle(PlayerPedId()) and PedHealth >= 10 and not ProgressBarExists() then
                                                ...
                                                RemoveProgressBar()
                                                AeroEvent("sCore:FarmLabo", v.labocolor, v.itemwin, v.itemcountwin, v.itemrequired, v.itemcountrequired, v.itemrequired2, v.itemcountrequired2)
                                                Wait(1500)
                                            end
                                        else
                                    ...
                                    if not IsPedSittingInAnyVehicle(PlayerPedId()) and PedHealth >= 10 and not ProgressBarExists() then
                                        ...
                                        RemoveProgressBar()
                                        AeroEvent("sCore:FarmLabo", v.labocolor, v.itemwin, v.itemcountwin, v.itemrequired, v.itemcountrequired, nil, nil)
                                        Wait(1500)
                                    end
                                end
                            else
                        ...
                        if not IsPedSittingInAnyVehicle(PlayerPedId()) and PedHealth >= 10 and not ProgressBarExists() then
                            ...
                            RemoveProgressBar()
                            AeroEvent("sCore:FarmLabo", v.labocolor, v.itemwin, v.itemcountwin, nil, nil, nil, nil)
                            Wait(1500)
                        end
                    end
                end
                            ...
                            if GetDistanceBetweenCoords(myCoords, labCoords.x, labCoords.y, labCoords.z,  true) < 2.0 then
                                ...
                                if v.owner == nil then
                                    ...
                                    if v.owner == ESX.PlayerData.job2.name then
                                        ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~g~entrer~s~ dans le laboratoire.")
                                        if IsControlJustReleased(0, 38) then
                                            AeroEvent("sLaboratoire:Enter", GetEntityCoords(PlayerPedId()), v)
                                        end
                                    end
                                end
                            end
...
AddEventHandler("sLaboratoire:StartAttack",function(jobname)
    if not startattack then
        ...
        for k,v in pairs(lablist) do
            AeroEvent("sLaboratoire:NotifyAttack",v.owner, v.ownerName, zone)
        end
        ...
        for k,v in pairs(lablist) do
            if not timeoutlab then
                AeroEvent("sLaboratoire:updateAttack", v.id, v.name, ESX.PlayerData.job2.name, ESX.PlayerData.job2.label)
                timeoutlab = true
                ...
                for k,v in pairs(champs) do
                    if #(myCoords-v.coords) < v.radius then 
                        ...
                        if IsControlJustReleased(0, 54) then
                            ...
                            AeroEvent("sDrugs:addItem", v.item)
                            onFarming = false
