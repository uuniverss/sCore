...
function openBuyMenu(table)
    if openedBuyMenu then
        ...
        buyMenu:SetSubtitle("~g~"..table.name)
        Citizen.CreateThread(function()
            getLaboratoireESX()
            while openedBuyMenu do
                Wait(1.0)
                RageUI.IsVisible(buyMenu, false, false, true, function()
                    RageUI.Separator("Prix du ~g~"..table.name.."~s~ "..table.price.."~g~$~s~")
                    RageUI.Button("Visiter", nil, {RightLabel = "~g~Entrer~s~ →"}, true, function(_, _, Select)
                        if (Select) then
...
local visiteMenu = RageUI.CreateMenu("", "Visite : ~g~Laboratoire", 80, 95)
        ...
        Citizen.CreateThread(function()
            while openedVisiteMenu do
                ...
                RageUI.IsVisible(visiteMenu, false, false, true, function()
                    RageUI.Separator("Laboratoire n°~g~"..table.id)
                    RageUI.Separator("~g~"..table.name.."~s~ - "..table.price.."~g~$~s~")
