...
AddEventHandler("Receleur:SellItem",function(itemRemove,countRemove,itemRemoveLabel,zone)
    ...
    local xPlayer = ESX.GetPlayerFromId(source)
    if #(GetEntityCoords(GetPlayerPed(xPlayer.source))-vector3(-3859.0014648438,-5719.1083984375,21.0)) > 1.5 then
...
AddEventHandler('Sneakyreceleur:buyItem', function(itemName, amount,zone)
	...
	if xPlayer.getAccount('cash').money >= money then
		if xPlayer.canCarryItem(itemName, amount) then
			...
			TriggerClientEvent('esx:showNotification', source, "Vous avez acheté ~g~<C>"..ESX.GetItemLabel(itemName).."<C> ~s~ pour <C>"..money.."~s~~g~$~s~<C>.")
		else
