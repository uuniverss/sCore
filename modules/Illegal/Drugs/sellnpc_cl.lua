...
function Drugs:StartBoucleForSelling()
    ...
    while true do
        ...
        while deliveryPos == nil do
            Wait(0)
            Drugs:DrawText("Vous êtes a la recherche de ~g~clients~s~.", 1)
        ...
        if deliveryPos ~= nil and delivery == nil then
            delivery = {
                point = deliveryPos, 
                blip = Drugs:CreateBlip(deliveryPos, {501, 2, "Livraison", 0.6}),
                entity = nil
            }
            ...
            Interval = 0
            Drugs:DrawText("Un point de livraison a été marqué dans la ~g~zone~s~.", 1)
            if #(GetEntityCoords(PlayerPedId())-delivery["point"]) < 3.0 then
                ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~g~déposer~s~ votre ~g~livraison~s~.")
                if IsControlJustReleased(0, 54) then
                    ...
                    if random == 3 then
                        AeroEvent("sCall:SendCallMsg", "Vente de drogue en cours", GetEntityCoords(PlayerPedId()), "police", false)
                        AeroEvent("sCall:SendCallMsg", "Vente de drogue en cours", GetEntityCoords(PlayerPedId()), "lssd", false)
                    end
                    AeroEvent("Drugs:Sell", playerHasItem)
                    delivery = nil
                end
