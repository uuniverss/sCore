...
AddEventHandler("sVip:NotifyVip",function(txtday, txthrs, txtminutes)
    if vipExpiration then
        ...
        ESX.ShowNotification("Il vous reste ~g~"..txtday.."~s~ jours, ~g~"..txthrs.."~s~ heures et ~g~"..txtminutes.."~s~ minutes avant l'expiration de votre VIP.")
    end
end)
...
function OpenBoutiqueMenuRageUIMenu()
    ...
    if sBoutique.Menu then 
        ...
        RMenu.Add('boutique', 'history', RageUI.CreateSubMenu(RMenu:Get("boutique", "main"),"", "Boutique de ~s~Ever ~g~Life"))
        RMenu.Add('boutique', 'itemmenu', RageUI.CreateSubMenu(RMenu:Get("boutique", "main"),"", "Boutique de ~s~Ever ~g~Life"))
        RMenu:Get('boutique', 'main'):SetSubtitle("Boutique de ~s~Ever ~g~Life")
        ...
        Citizen.CreateThread(function()
			while sBoutique.Menu do
                RageUI.IsVisible(RMenu:Get('boutique', 'main'), true, false, true, function()
                    if id == 0 then
                        RageUI.Separator("Votre ~g~ID Boutique~s~ : ~r~Compte FiveM non lié")
                    else
                        RageUI.Separator("Votre ~g~ID Boutique~s~ : "..id)
                    end
                    RageUI.Separator("Vos ~g~eCoins~s~ : "..points)
                    if vipRank == 0 then
                        vipText = "Aucun"
                    ...
                    RageUI.Button("Transactions", "Historique de vos achats sur la boutique en jeux", {RightLabel = "→ ~g~Regarder ",RightBadge = RageUI.BadgeStyle.Coins}, true, function(h,a,s)
                        if s then
                    ...
                    if (#category > 0) then
                        for i, v in pairs(category) do
                            RageUI.Button(v.name, v.descriptions, {RightLabel = "→ ~g~Acheter ",RightBadge = RageUI.BadgeStyle.Coins}, true, function(h,a,s)
                                if s then
                    ...
                    if (#items > 0) then
                        for i, v in pairs(items) do
                            if v.category == 2 then
                                RageUI.Button(v.name, nil, { RightLabel = string.format("~g~%s", v.price),RightBadge = RageUI.BadgeStyle.Coins }, true, function(h,a,s)
                                    if s then
                                        ...
                                        if (points >= v.price) then
                                            ...
                                            RageUI.GoBack()
                                            AeroEvent("sCore:deletecoins",v.descriptions, v.price, v.name)
                ...
                RageUI.IsVisible(RMenu:Get('boutique', 'history'), true, false, true, function()
                    if (#history > 0) then
                        RageUI.Separator("↓ ~g~Votre historique~s~ ↓")
                        for i, v in pairs(history) do
                            local label;
                            if (tonumber(v.price) == 0) then
                                label = string.format("%s ~g~coins", v.points)
                            else
                                label = string.format("%s (%s%s) ~g~coins", v.points, v.price, v.currency);
                            end
...
Keys.Register('F1','OpenBoutiqueMenuRageUIMenu', 'Boutique ~s~Ever ~g~Life', function()
