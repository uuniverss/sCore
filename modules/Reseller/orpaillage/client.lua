...
local config = {
    ...
    menu = {pos = vector3(1073.7261962891,-2008.9866943359,32.084953308105), message = "Appuyez sur ~INPUT_CONTEXT~ pour vendre vos ~g~pépites d'or~s~."},
    items = {
        {
            itemReseller = "ors",
            label = "Pépites d'or",
            ...
            for k,v in pairs(config.items) do
                RageUI.Button("Vendre des "..config.color..""..v.label, nil, {RightLabel = v.priceReseller.."~g~$~s~"}, true, function(h, a, s)
                    if s then
                        ...
                        AeroEvent("sOrpaillage:sellItem", v.itemReseller, count)
                    end
                end)
            end
        end)
