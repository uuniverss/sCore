...
AddEventHandler("Sneakylife:ShowCard", function(identity,licenses)
    ...
    if licenses == nil or #licenses == 0 then
      ...
      for i = 1, #licenses, 1 do
          ESX.ShowNotification("Vos licenses acquises ↓~n~~g~"..licenses[i].type)
      end
    end
    ESX.ShowNotification("Identité :\nPrénom : ~g~"..identity.firstname.."\n~s~Nom : ~g~"..identity.lastname.."\n~s~Date de naissance : ~g~"..identity.birthday.."\n~s~Taille : ~g~"..identity.height.."~s~")
end)
