	...
	if xPlayer.job.name == society.name then
		TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
			if amount > 0 and account.money >= amount then
				...
				TriggerClientEvent("RageUI:Popup", source, {message="Société : ~g~"..society.label.."~s~ \nType : ~g~Retrait~s~\nMontant : ~g~$"..ESX.Math.GroupDigits(amount)})
				SendLogs(3066993, society.label.." - Retrait","**"..GetPlayerName(_source).."** vient de retirer ***"..amount.."$ ***" ,"https://discord.com/api/webhooks/1151209657529016330/gnfFYzlg3_JBEfX5OcByGdsqWphhZZQ6khBhhFHuDgPq-llHhqY6rjkS8ZYLBETnISyP")
			else
				TriggerClientEvent("RageUI:Popup", source, {message="~g~Action impossible"})
			end
		end)
	else
...
AddEventHandler('pSociety:depositMoney', function(society, amount)
	...
	if xPlayer.job.name == society.name then
		if amount > 0 and xPlayer.getAccount('cash').money >= amount then
			...
			TriggerClientEvent("RageUI:Popup", source, {message="Société : ~g~"..society.label.."~s~ \nType : ~g~Dépôt~s~\nMontant : ~g~$"..ESX.Math.GroupDigits(amount)})
			SendLogs(3066993, society.label.." - Dépot","**"..GetPlayerName(_source).."** vient de déposer ***"..amount.."$ ***" ,"https://discord.com/api/webhooks/1151209482110640269/Sw75KSC7XTybf181QQWvtYs65BQpGTw5lflGzw5_Rxpai7x2dJXLnSJJItRQksf-j-S4")
...
AddEventHandler('pSociety:washMoney', function(society, amount)
	...
	if xPlayer.job.name == society then
		if amount and amount > 0 and account >= amount then
			TriggerEvent('esx_addonaccount:getSharedAccount', societyy.account, function(sctyacc)
				xPlayer.removeAccountMoney('dirtycash', amount)
				sctyacc.addMoney(amount*0.9)
				TriggerClientEvent("RageUI:Popup", source, {message="~r~Vous avez blanchis :~s~ \n$"..ESX.Math.GroupDigits(amount)})
				SendLogs(3066993, societyy.label.." - Dépot","**"..GetPlayerName(_source).."** vient de déposer ***"..amount.."$ ***" ,"https://discord.com/api/webhooks/1151209482110640269/Sw75KSC7XTybf181QQWvtYs65BQpGTw5lflGzw5_Rxpai7x2dJXLnSJJItRQksf-j-S4")
			end)
		else
			TriggerClientEvent("RageUI:Popup", source, {message="~g~Action impossible"})
		end
	...
	if isBoss then
		...
		if grade ~= tonumber(getMaximumGrade(job)) or job == "unemployed" then
			if xTarget then
				...
				TriggerClientEvent("RageUI:Popup", xTarget, {message="~g~Votre profession à évoluée"})
				TriggerClientEvent("RageUI:Popup", source, {message="~g~Profession modifiée !"})
...
AddEventHandler("pSociety:RequestSetRecruit", function(target, job)
	...
	if xPlayer.job.grade_name == 'boss' then
		...
		TriggerClientEvent("RageUI:Popup", source, {message="~g~Demande envoyée."})
	end
end)
...
AddEventHandler("pSociety:SetJob", function(job, grade)
	...
	xPlayer.setJob(job, grade)
	TriggerClientEvent("RageUI:Popup", source, {message="~g~Votre profession à évoluée"})
end)