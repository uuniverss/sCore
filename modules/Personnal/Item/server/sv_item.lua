...
AddEventHandler('esx_repairkit:removeKit', function()
	...
	if not InfiniteRepairs then
		...
		TriggerClientEvent('esx:showNotification', _source, "Vous avez utilisé un ~g~kit de réparation")
	end
end)
	...
	if item_name ~= "diable" then
		...
		if xPlayer.canCarryItem("diable", 1) then
			...
			if itemdiable == 0 then
				...
				TriggerClientEvent("esx:showNotification",source,"Vous venez de plier le ~g~<C>diable</C>~s~.")
			end
		else
...
AddEventHandler('SneakyLife:UseItemFinish', function(item_name)
	...
	TriggerClientEvent("esx:showNotification",source,"Vous venez d'utiliser un(e) ~g~"..ESX.GetItemLabel(item_name).."~s~.")
end)