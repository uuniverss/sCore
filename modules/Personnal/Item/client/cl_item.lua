...
AddEventHandler('esx_armour:armor', function()
	...
	if GetPedArmour(plyPed) == 100 then
		...
		AnimGilet()
		SneakyEvent('esx_armour:armorremove')
		ESX.ShowNotification("Tu as utilisé un ~g~gilet~s~ par balle.")
	end
end)
...
function AnimGilet()
	...
	TriggerEvent('Sneakyskinchanger:getSkin', function(skin)
		if skin.sex == 0 then
			TriggerEvent('Sneakyskinchanger:loadClothes', skin, {['bproof_1'] = 16,  ['bproof_2'] = 2})
		else
			TriggerEvent('Sneakyskinchanger:loadClothes', skin, {['bproof_1'] = 19,  ['bproof_2'] = 2})
		end
	end)
...
AddEventHandler('esx_repairkit:onUse', function()
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		...
		if DoesEntityExist(vehicle) then
			if IgnoreAbort then
				SneakyEvent('esx_repairkit:removeKit')
			end
			...
			Citizen.CreateThread(function()
				...
				if CurrentAction ~= nil then
					...
					ESX.ShowNotification("Vous avez fini de ~g~réparer")
					CurrentAction = nil
...
AddEventHandler("Sneaky:Uselockpick",function()
	...
	if DoesEntityExist(vehicle) then
		...
		Citizen.CreateThread(function()
            ...
            if chance >= 70 then
                AeroEvent("Sneakyremovelockpick")
            end
		end)
	else
        ...
        while true do
            ...
            for k,v in pairs(vehicle) do 
                ...
                if dst <= 1.8 and plaque == "BMX" then 
                    ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~g~ramasser le bmx.")
    ...
    while IsEntityAttachedToEntity(diable, PlayerPedId()) do
        ...
        if IsControlJustPressed(0, 73) then
            ...
            if PlayerWeight < 45 then
                AeroEvent("sCore:diableSystem","diable")
                ClearPedTasksImmediately(PlayerPedId())
                DetachEntity(diable, true, true)
                DeleteEntity(diable)
            else
...
function AttachCaseToPed(item_name)
    ...
    if Props.advanced then
        ...
        DrawTopNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~g~utiliser votre "..Props.name.."~w~.")
        Citizen.CreateThread(function()
...
function RepairVehicle(type, Entity)
    ...
    DontSpam = false
    AeroEvent("SneakyLife:UseItemFinish", remove_itemname)
...
AddEventHandler("SneakyLife:UseItemTires", function()
    ...
    ShowAboveRadarMessage("Vous avez réparé une ~g~roue du véhicule~s~.")
    AeroEvent("SneakyLife:UseItemFinish", "pneu")
end)
TimeWash = 6000
...
AddEventHandler("SneakyLife:UseItemWash", function()
    ...
    AeroEvent("SneakyLife:UseItemFinish", "kit_de_lavage")
end)
