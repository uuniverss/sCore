...
AddEventHandler('announce:pizza', function(announce)
    if announce == 'ouvert' then
        ESX.ShowAdvancedNotification("~g~Drusilla's~s~ Pizza", '~g~Informations', "- Drusilla's Pizza ~g~ouvert~s~~n~- Horaire : ~g~Maintenant", "CHAR_PIZZA", 1)
    elseif announce == 'fermeture' then
        ESX.ShowAdvancedNotification("~g~Drusilla's~s~ Pizza", '~g~Informations', "- Drusilla's Pizza ~r~fermé~s~~n~- Horaire : ~g~Maintenant", "CHAR_PIZZA", 1)
    end
end)
