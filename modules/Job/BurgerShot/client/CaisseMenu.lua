...
AddEventHandler('announce:burgershot', function(announce)
    if announce == 'ouvert' then
        ESX.ShowAdvancedNotification("Burger Shot", '~g~Informations', "- Burger Shot ~g~ouvert~s~~n~- Horaire : ~g~Maintenant", "CHAR_BURGERSHOT", 1)
    elseif announce == 'fermeture' then
        ESX.ShowAdvancedNotification("Burger Shot", '~g~Informations', "- Burger Shot ~r~fermé~s~~n~- Horaire : ~g~Maintenant", "CHAR_BURGERSHOT", 1)
    end
end)
