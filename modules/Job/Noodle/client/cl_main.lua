...
AddEventHandler('announce:noodlee', function(announce)
    if announce == 'ouvert' then
        ESX.ShowAdvancedNotification('Noodle Exchange', '~g~Informations', "- Noodle Exchange ~g~ouvert~s~~n~- Horaire : ~g~Maintenant", "CHAR_NOODLE", 1)
    elseif announce == 'fermeture' then
        ESX.ShowAdvancedNotification('Noodle Exchange', '~g~Informations', "- Noodle Exchange ~r~fermé~s~~n~- Horaire : ~g~Maintenant", "CHAR_NOODLE", 1)
    end
end)
...
local MenuNoodle = {}
RMenu.Add('noodlemenu', 'main', RageUI.CreateMenu("", "~g~Noodle", nil, nil,"root_cause","noodle"))
...
function OpenNoodleActionMenuRageUIMenu()
    local mainMenu = RageUI.CreateMenu("", "~g~Noodle Exchange", 0, 0,"root_cause","noodle")
