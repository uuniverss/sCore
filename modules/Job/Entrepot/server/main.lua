...
local StockageEntreprise = {
    ...
    ["burgershot"] = {
        ...
        Label = "Burger Shot",
        Description = "Aliments du Burger Shot",
        Texture = "root_cause",
        Banner = "burgershot",
        Bucket = 150,
        Items = {
...
AddEventHandler("kEntrepot:enterStock", function(table)
    ...
    SetPlayerRoutingBucket(source, table.Bucket)
...
ESX.RegisterServerCallback("kEntrepot:buyItems", function(source, cb, item, price, job)
    if source then
        ...
        if xPlayer.getAccount("cash").money >= price then
            cb("money")
            TriggerClientEvent("esx:showNotification", source, "Vous venez d'acheter ~g~<C>1x "..ESX.GetItemLabel(item).."~s~ pour "..price.."~g~$<C>~s~ !")
            xPlayer.removeAccountMoney("cash", price)
            xPlayer.addInventoryItem(item, 1)
            exports.sCore:SendLogs(1752220,"Achat d'item (entrepôt)",""..GetPlayerName(source).." vient d'acheter "..count.." de "..ESX.GetItemLabel(item).." avec l'entreprise : "..job.." \n License de la source : "..xPlayer.identifier,"https://discord.com/api/webhooks/1151183212857540608/IpV97byc5hAXJ4_3Ju1dWQgy3f21zJbnKTjx44ubwiiYYvq3geBoHRKSgXeU43c6RhRo")
        else
