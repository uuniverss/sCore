...
mayansmVestiaire = {}
RMenu.Add('mayansmvestiaire', 'main', RageUI.CreateMenu("", "",10, 200,"root_cause","mayans"))
RMenu:Get('mayansmvestiaire', 'main'):SetSubtitle("~g~Actions disponibles")
...
local MecanoKitRepa = {}
RMenu.Add('mayansmshop', 'main', RageUI.CreateMenu("", "",10, 200,"root_cause","mayans"))
RMenu:Get('mayansmshop', 'main'):SetSubtitle("~g~Produits disponibles")
        ...
        Citizen.CreateThread(function()
			while MecanoKitRepa.Menu do
                RageUI.IsVisible(RMenu:Get('mayansmshop', 'main'), true, false, true, function()
                    RageUI.Button("Kit de réparation", false, {RightLabel = "→ Acheter 2300~g~$"}, true, function(h,a,s)
					if s then
...
mayansmGarage = {}
RMenu.Add('mayansmgarage', 'main', RageUI.CreateMenu("", "",10, 200,"root_cause","mayans"))
RMenu:Get('mayansmgarage', 'main'):SetSubtitle("~g~Actions disponibles")
...
local mayansmClean = {}
RMenu.Add('mayansmclean', 'main', RageUI.CreateMenu("", "",10, 200,"root_cause","mayans"))
...
function OpenMayansmActionMenuRageUIMenu()
    ...
    if mayansmActionMenu.Menu then 
        ...
        RMenu.Add('mayansmactionmenu', 'main', RageUI.CreateMenu("", "", 0, 0,"root_cause","mayans"))
						...
						if s then
							...
							if DoesEntityExist(vehicle) then
								...
								Citizen.CreateThread(function()
									...
									ESX.ShowNotification("mayansm véhicule dévérouillé")
								end)
							else
...
AddEventHandler('mayansm:targetAnnounce', function(announce)
    if announce == 'ouvert' then
        ESX.ShowAdvancedNotification("Mayans Motors", '~g~Informations', "- Mécano ~g~ouvert~s~~n~- Horaire : ~g~Maintenant", "CHAR_HARMONY", 1)
    elseif announce == 'fermeture' then
        ESX.ShowAdvancedNotification("Mayans Motors", '~g~Informations', "- Mécano ~r~fermé~s~~n~- Horaire : ~g~Maintenant", "CHAR_HARMONY", 1)
    end
end)