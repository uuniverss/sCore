...
local function ThinkDeath(player)
    if Death.PlayerDead then
        ...
        if IsControlJustPressed(1, 47) and Death.Call then 
            ESX.ShowNotification("Vous avez appelé les ~g~secours~s~.")
            AeroEvent("sCall:SendCallMsg", "Une personne est au sol.", GetEntityCoords(PlayerPedId()), "ambulance", true)
            Death.Call = false 
        end
        if GetGameTimer() >= Death.deathTime and Death.PlayerDead then
            SetTimecycleModifier("damage")
            Death:DrawTextScreen(0.85, 0.82, 0.8, "Appuyez sur ~g~E~s~ pour contacter les internes.", 4, 0)
            if IsControlJustPressed(1, 38) then 
                ...
                Wait(500)
                DoScreenFadeIn(1000)
                TriggerEvent('esx_status:healPlayer')
                ...
                PlaySoundFrontend(-1, "1st_Person_Transition", "PLAYER_SWITCH_CUSTOM_SOUNDSET", 0)
                AeroEvent("Sneaky:StopDeadPlayer")
                SneakyEvent("esx_ambulancejob:payrevive",500)
                ESX.ShowNotification("~g~Médecin~s~\nVous avez été soigné par un médecin en urgence , merci de ne pas forcer sur vos blessures")
    ...
    elseif Death.PlayerKO then 
        ...
        if GetGameTimer() >= Death.deathTime and Death.PlayerKO then
            Death:DrawTextScreen(0.85, 0.82, 0.8, "Appuyez sur ~g~E~s~ pour vous relever.", 4, 0)
            if IsControlJustPressed(1, 38) then
                local pPed = player
                Death.PlayerKO = false 
                DoScreenFadeOut(1000)
...
AddEventHandler('SneakyLife:onPlayerDied', function(victimEntity, attackEntity)
    if victimEntity and not Death.PlayerDead and not Death.PlayerKO and IsEntityDead(victimEntity) then
        ...
        deadtimeout = true
        isPlayerAbuse = false
        AeroEvent("Sneaky:DeadPlayer")
        ESX.ShowNotification("Appuyez sur ~g~G~s~ pour contacter les ~g~secours~s~.")
        Death:Revive()
        SetTimecycleModifier("damage")
        SetPedToRagdoll(PlayerPedId(),1000, 1000, 0, 0, 0, 0)
...
AddEventHandler('SneakyLife:onPlayerKilled', function(victimEntity, _z)
    if not Death.PlayerDead and not Death.PlayerKO then
        ...
        ESX.ShowNotification("Vous êtes tombé ~g~KO~s~.")
...
Citizen.CreateThread(function()
    while true do
        if not loadedcoma then
            Wait(5000)
            AeroEvent("sComa:RequestDeadStatut")
            loadedcoma = true
        else
            Wait(20000)
        end
...
AddEventHandler('SneakyLife:RevivePlayer', function()
    ...
    ESX.RemoveTimerBar()
    timerbaractive = false
    ShowNotification("~g~Réanimation\n~w~Vous venez d\'être réanimé.")
    SetTimeout(2000,function()
        deadtimeout = false
    end)
end)
