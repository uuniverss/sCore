	...
	positions = {
		["garage"] = {
			...
			vehicle = {
				...
				Vehicles = {
					{label = "Granger blindé", veh = "USSSgranger", stock = 5},
					{label = "Van banalisé", veh = "USSSvan", stock = 3},
                    {label = "Granger vip blindé", veh = "USSSvip", stock = 7},
                    {label = "Ambulance Personnel", veh = "USSShazard", stock = 1},
--                    {label = "USS Scout", veh = "USSSscout", stock = 10},
--                    {label = "USS Torrence", veh = "USSStorrence", stock = 10},
                    {label = "Limousine blindé", veh = "onebeast", stock = 1},
                    {label = "Granger blindé avec gyrophare", veh = "USSSgrangerbar", stock = 5}
				}
			},
			deleteVehicle = vector3(-575.38275146484,-169.14524841309,37.608043670654-0.9)
		}, -- Fin garagxe
		["armory"] = {
			armoryPos = vector3(-517.190429688,-172.229071044922,42.7299194335938-0.2),
			acces = {["agentsecurite"] = true, ["superviseursecurite"] = true, ["directeursecurite"] = true},
			weapons = {
                [1] = {name = "WEAPON_FLASHLIGHT", label = "Lampe Torche"},
				[2] = {name = "WEAPON_COMBATPISTOL", label = "Glock 17"},
                [3] = {name = "WEAPON_SPECIALCARBINE", label = "G36"},
			}
		}, -- Fin armory
--[[        ["avocat"] = {
            ...
            Positions = {
                {pos = vector3(-536.98117675781,-196.4844299316,38.222400665283-0.9)}
            },
		}, -- Fin avocat --]]
        ["service"] = {
            Positions = {
                {pos = vector3(-551.18585205078,-189.53569030762,38.219657897949-0.9)}
            },
		}
	},
}
    ...
    while true do
		...
		if ESX.PlayerData.job.name == Gouvernement.job then
			...
			elseif #(myCoords-Gouvernement.positions["garage"].deleteVehicle) < 3.5 then
				nofps = true
				ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ranger ~g~le véhicule de service~s~.")
			...
			if Gouvernement.positions["armory"].acces[ESX.PlayerData.job.grade_name] ~= nil then
				...
				if #(myCoords-Gouvernement.positions["armory"].armoryPos) < 1.5 then
					nofps = true
					ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ouvrir ~g~l'armurerie~s~.")
					if IsControlJustReleased(0, 38) then
                        if onService then
						    openArmory()
                        else
...
--[[            elseif Gouvernement.positions["avocat"].acces[ESX.PlayerData.job.grade_name] ~= nil then
                ...
                for k,v in pairs(Gouvernement.positions["avocat"].Positions) do
                    if #(myCoords-v.pos) < 1.5 then
                        nofps = true
                        ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ouvrir ~g~le menu gestion avocat~s~.")
                        if IsControlJustReleased(0, 38) then
                            if onService then
                                openAvocat()
                            else
