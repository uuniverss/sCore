...
AddEventHandler('announce:yellowjack', function(announce)
    if announce == 'ouvert' then
        ESX.ShowAdvancedNotification('YellowJack', '~g~Informations', "- YellowJack ~g~ouvert~s~~n~- Horaire : ~g~Maintenant", "CHAR_YELLOWJACK", 1)
    elseif announce == 'fermeture' then
        ESX.ShowAdvancedNotification('YellowJack', '~g~Informations', "- YellowJack ~r~fermé~s~~n~- Horaire : ~g~Maintenant", "CHAR_YELLOWJACK", 1)
    end
end)
...
RMenu.Add('yellowjackmenu', 'main', RageUI.CreateMenu("", "~g~Yellowjack", nil, nil,"root_cause","yellowjack"))
...
function OpenYellowjackActionMenuRageUIMenu()
    local mainMenu = RageUI.CreateMenu("", "~g~YellowJack", 0, 0,"root_cause","yellowjack")
...
RMenu:Get('garageyellowjack', 'main'):SetSubtitle("~g~Yellowjack~s~")
