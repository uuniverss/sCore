...
Keys.Register('F6','InteractionsJob', 'Menu job', function()
    if not bloquertouchejojo and not toucheBloqueKadir and not exports.inventaire:GetStateInventory() then
        if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == "police" or ESX.PlayerData.job.name == "lssd" or ESX.PlayerData.job.name == "fbi" then
    ...
    if not toucheBloqueKadir and not bloquertouchejojo and not exports.inventaire:GetStateInventory() then
        ...
        elseif ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == "mayansm" then
            if CheckServiceMayansm() then
                OpenMayansmActionMenuRageUIMenu()
            else
                ESX.ShowNotification("Vous n'êtes pas en ~r~service~s~.")
            end
        end
