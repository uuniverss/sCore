    ...
    allowJobs = {
      ["ltd_sud"] = {
        Announce = {
            ["open"] = {
                title = "LTD Sud",
                subtitle = "~g~Information~s~",
                message = "Nous sommes actuellement ~g~ouverts~s~, venez nous rendre ~g~visite~s~ au sud !",
                banner = "CHAR_LTD"
            },
            ["close"] = {
                title = "LTD Sud",
      ...
      ["ltd_nord"] = {
        Announce = {
            ["open"] = {
                title = "LTD Nord",
                subtitle = "~g~Information~s~",
                message = "Nous sommes actuellement ~g~ouverts~s~, venez nous rendre ~g~visite~s~ au nord !",
                banner = "CHAR_LTD"
            },
            ["close"] = {
                title = "LTD Nord",
...
AddEventHandler("sLtd:StateService", function()
    ...
    if LTD.onService[source] then
        notifToEmployes("~g~"..GetIdentityPlayer(xPlayer.identifier).."~s~ à ~r~quitté~s~ son ~o~service~s~ !", LTD.onService[source].job)
        LTD.onService[source] = nil;
        checkThePlayerLtd()
    else
        LTD.onService[source] = {job = xPlayer.job.name}
        notifToEmployes("~g~"..GetIdentityPlayer(xPlayer.identifier).."~s~ à ~g~pris~s~ son ~o~service~s~ !", LTD.onService[source].job)
        checkThePlayerLtd()
    end
end)
...
AddEventHandler("sLtd:buyItem", function(itemName)
    ...
    if xPlayer.getAccount('cash').money > infosItem.price then
        ...
        TriggerClientEvent("esx:showNotification", source, "Vous avez acheté ~g~x1 "..ESX.GetItemLabel(itemName).."~s~ pour "..infosItem.price.."~g~$~s~ !")
