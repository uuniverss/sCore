            ...
            ["garage"] = {
                ...
                vehicle = {
                    ...
                    Vehicles = {
                        ...
                        {label = "Police Pinnacle", veh = "pinnaclep", stock = 8},
                        ...
                        {label = "Police Intruder", veh = "police6", stock = 4},
                        {label = "Dominator Banalisé", veh = "LSPDumkdominator", stock = 2},
                        {label = "Bravado Buffalo Banalisé", veh = "LSPDumkbuffalo09", stock = 2},
                        {label = "Vapid Cruiser Banalisé", veh = "police3umk", stock = 2},
                        {label = "Vapid Stanier Banalisé", veh = "LSPDumkstanier05", stock = 2},
                        {label = "Fugitive Banalisé", veh = "LSPDumkfugitive", stock = 2},
                        {label = "Felon Banalisé", veh = "policefelon", stock = 2},
                        {label = "Vapid Scout Banalisé", veh = "LSPDumkscout14", stock = 1},
                        ...
                        {label = "Vapid Scout TD", veh = "LSPDscout16", stock = 2},
                        {label = "Vapid Scout Police Academy", veh = "pscout", stock = 4},
                        {label = "Police Everon", veh = "LSPDeveron", stock = 2},
                        {label = "Declasse Alamo", veh = "LSPDalamo14", stock = 2},
                        {label = "Declasse Alamo Valor", veh = "LSPDalamo16", stock = 2},
                        {label = "Police Speedo", veh = "polnspeedo", stock = 2},
                        ...
                        {label = "Stalker Swat", veh = "LSPDumklandstalkerxl", stock = 2},
                        {label = "Granger Swat", veh = "LSPDgrangertac", stock = 2},
...
AddEventHandler('renfortpolice:setBlip', function(job, coords, raison)
	if raison == 'petit' then
		...
		ESX.ShowAdvancedNotification(job..' INFORMATIONS', '~g~Demande de renfort', 'Demande de renfort demandé.\nRéponse: ~g~CODE-2\n~w~Importance : ~g~Légère.', 'CHAR_CALL911', 8)
		Wait(1000)
...
AddEventHandler('Sneaky:ClientMessage', function(job,msg)
    if job == "police" then
        label = "~g~Los Santos Police Department~s~"
    else
        label = "~g~Los Santos Sheriff's Department~s~"
    end
...
local function openGiletArmory(job)
    ...
    local gilet1Menu = RageUI.CreateSubMenu(mainMenu,"", "~g~Gilet par balle~s~")
    local gilet2Menu = RageUI.CreateSubMenu(mainMenu,"", "~g~Gilet par balle~s~")
    RageUI.Visible(mainMenu, not RageUI.Visible(mainMenu))
    while mainMenu do
        Wait(0)
        RageUI.IsVisible(mainMenu, true, false, false, function()
            ESX.ShowHelpNotification("Merci de prendre un seul gilet\nqui correspond à votre ~g~grade")
            ...
            RageUI.Button("Fouiller", nil, {RightLabel = "→"}, closestPlayer ~= -1 and closestDistance <= 3.0,function(h,a,s)
                if s then
                    ...
                    if closestPlayer ~= -1 and closestDistance <= 3.0 then
                        ...
                        ESX.ShowNotification("~g~Police~s~\nPersonne à proximité")
                    end
                end
            end, inventoryPlayer)
        ...
        RageUI.IsVisible(licenses, true, false, true, function()
            if resultLicenses == nil then
                ...
                if #resultLicenses == 0 then
                    RageUI.Separator("")
                    RageUI.Separator("~g~Aucune license(s) !")
                    RageUI.Separator("")
                else
                    RageUI.Separator("↓ ~g~License(s)~s~ ↓")
                    for k,v in pairs(resultLicenses) do
                        RageUI.Button(v.label, nil, {RightLabel = "~g~Enlever~s~ →"}, closestPlayer ~= -1 and closestDistance <= 3.0, function(h, a, s)
                            if s then
                                SneakyEvent("esx_license:removeLicense", GetPlayerServerId(closestPlayer), v.type)
                                ESX.ShowNotification("Vous avez enlevé la license ~c~"..v.label.."~s~ à ~g~"..beforeInfo.firstname.."~s~ !")
...
--        RMenu:Get('fourriere', 'main'):SetSubtitle("~g~Menu personnel")
...
Citizen.CreateThread(function()
    while true do
        ...
        if jobdist <= 10.0 and Config.jeveuxmarker then
            ...
            if jobdist <= 1.0 then
                Timer = 0
                ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour voir les ~g~véhicules saisies")
...
Citizen.CreateThread(function()
    while true do
        local Timer = 500
        if ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' or ESX.PlayerData.job.name == 'mecano' then
        local plycrdjob = GetEntityCoords(GetPlayerPed(-1), false)
        local jobdist = Vdist(plycrdjob.x, plycrdjob.y, plycrdjob.z, Config.mettrefourriere.position.x, Config.mettrefourriere.position.y, Config.mettrefourriere.position.z)
            if jobdist <= 2.0 then
                Timer = 0
                ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour mettre un ~g~véhicule en saisie")
                if IsControlJustPressed(1,51) then
            ...
            RageUI.Button("Déposer ses armes de services", nil, {RightLabel = "~g~Déposer →"}, true, function(h,a,s)
                if s then
                    ...
                    if HasPedGotWeapon(PlayerPedId(), "WEAPON_FLASHLIGHT", false) or HasPedGotWeapon(PlayerPedId(),"WEAPON_NIGHTSTICK",false) or HasPedGotWeapon(PlayerPedId(),"WEAPON_STUNGUN",false) or HasPedGotWeapon(PlayerPedId(),"WEAPON_COMBATPISTOL",false) or HasPedGotWeapon(PlayerPedId(),"WEAPON_PUMPSHOTGUN",false) or HasPedGotWeapon(PlayerPedId(),"WEAPON_CARBINERIFLE",false) then
                        ...
                        ESX.ShowNotification("~g~Armurerie~s~\nVous n'avez pas d'arme à déposer")
                    end
                end
            end)
...
local function openClothes(type, job)
    local mainMenu = RageUI.CreateMenu("", "~g~Vestiaire", nil, nil, "root_cause", job.texture)
    RageUI.Visible(mainMenu, not RageUI.Visible(mainMenu))
    ...
    if type == "police" then
        for k,v in pairs(clothes) do
            subMenus[v.submenu] = RageUI.CreateSubMenu(mainMenu, "", "~g~Vestiaire~s~ : "..v.label, nil, nil, "root_cause", job.texture)
        end
    end
        ...
        for k,v in pairs(PoliceJob) do
            ...
            if ESX.PlayerData.job.name == v.job then
                ...
                if #(myCoords-v.positions["garage"].menuPos) < 1.5 then
                    nofps = true
                    ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ouvrir le ~g~garage~s~.")
                ...
                if #(myCoords-v.positions["garagehelico"].menuPos) < 1.5 then
                    nofps = true
                    ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour prendre un ~g~hélico~s~.")
                    if IsControlJustReleased(0, 38) then
                        if onService then
                            openGarageHelico(v)
                        else
                ...
                elseif #(myCoords-v.positions["garagehelico"].deleteVehicle) < 3.5 then
                    nofps = true
                    ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ranger ~g~l'hélicoptère~s~.")
                    if IsControlJustPressed(0, 51) then
                        if onService then
                ...
                if #(myCoords-v.positions["armory"].giletPos) < 1.5 then
                    nofps = true
                    ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour accéder au ~g~gilet par balle~s~.")
                    if IsControlJustReleased(0, 38) then
                        openGiletArmory(v)
                    end
                end
                ...
                if #(myCoords-v.positions["armory"].armoryPos) < 1.5 then
                    nofps = true
                    ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ouvrir l'~g~armurerie~s~.")
                    if IsControlJustReleased(0, 38) then
                        if onService then
                            openArmory(v)
                        else
                ...
                if #(myCoords-v.positions["clothes"].menuPos) < 1.5 then
                    nofps = true
                    ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ouvrir le ~g~vestiaire~s~.")
                    if IsControlJustReleased(0, 38) then
