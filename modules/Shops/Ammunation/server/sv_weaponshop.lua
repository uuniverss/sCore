...
AddEventHandler('sAmmunation:buyWeapon', function(weaponName)
	...
	if xPlayer.getAccount('cash').money >= verifWeapon[weaponName].price then
		...
		TriggerClientEvent("esx:showNotification", _source, "Vous avez acheté ~g~x1 "..ESX.GetWeaponLabel(weaponName).."~s~ pour "..verifWeapon[weaponName].price.."~g~$~s~ !")
...
AddEventHandler('sAmmunation:buyItem', function(itemName)
	...
	if xPlayer.getAccount('cash').money >= verifWeapon[itemName].price then
		if xPlayer.canCarryItem(itemName, verifWeapon[itemName].limit) then
			...
			xPlayer.showNotification('Vous venez de acheter ~g~1x '..ESX.GetItemLabel(itemName)..'~s~ pour '..verifWeapon[itemName].price..'~g~$~s~ !')
		else
			xPlayer.showNotification('Vous portez trop de ~g~'..ESX.GetItemLabel(itemName)..'~s~ sur vous !')
		end
