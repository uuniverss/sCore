        ...
        Citizen.CreateThread(function()
			while Weapon.Menu do
                RageUI.IsVisible(RMenu:Get('weapon', 'main'), true, false, true, function()
                    --if XNL_GetCurrentPlayerXP() >= 600 then
                        if LicenseOwned == 1 then
                            ...
                            RageUI.Button("Acheter le permis port d'armes", "Allez voir la ~g~L.S.P.D~s~ !", {}, true, function(h,a,s)
                            end)
                ...
                RageUI.IsVisible(RMenu:Get('weapon', 'whitearms'), true, false, true, function()
                    RageUI.Button("Boite de 10 canettes", false, {RightLabel = "→ 1000~g~$"}, true, function(h,a,s)
                        if s then
                            SneakyEvent('sAmmunation:buyWeapon', "WEAPON_CANETTE", 1000)
                        end
                    end)
                    RageUI.Button("Boite de 10 bouteille usagé", false, {RightLabel = "→ 1100~g~$"}, true, function(h,a,s)
                        if s then
...
local WeaponShop = {
    ...
    {
        pos = vector3(16.343099594116,-1109.0218505859,29.797210693359),
        blip = {
            label = "Armurerie", 
            ID = 110, 
            Color = 40
