...
function GetNearbyPlayer(solo, other)
    ...
    if #oPlayer == 0 then
        ShowAboveRadarMessage("~g~Distance\n~w~Rapprochez-vous.")
        return false
    end
    if #oPlayer == 1 and other then
        return oPlayer[1]
    end
    ShowAboveRadarMessage("Appuyer sur ~g~E~s~ pour valider.\nAppuyer sur ~g~A~s~ pour changer de cible.\nAppuyer sur ~r~X~s~ pour annuler.")
    Citizen.Wait(100)
    local cBase = 1
    cWait = true
...
AddEventHandler("sBill:CreateBill", function(data)
    ...
    if raisn ~= nil and raisn ~= "" then
        if amount ~= nil and amount ~= "" and checknumber(amount) then
            ...
            AeroEvent("sBill:SendBill", Bill.Data)
            ClearPedTasks(pPed)
        else
...
AddEventHandler("sBill:GetBill", function(bill)
    local pPed = PlayerPedId()
    ShowAboveRadarMessage("Facture: ~g~"..bill.title.."~s~.\nMontant: ~g~"..bill.price.."$~s~.")
    ShowAboveRadarMessage("Accepter: ~g~B~s~ ou Refuser: ~r~X~s~.")
    Bill.OnThread = false
...
AddEventHandler('sBill:AlertBill', function(data)
    if data == 1 then 
        ShowAboveRadarMessage("~g~La personne a payé la facture.")
    elseif data == 2 then 
