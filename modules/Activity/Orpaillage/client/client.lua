...
local function UseShovel(Player)
	AeroEvent("sTerreActivity:ChangeState",true)
	...
	AeroEvent('sTerreActivity:HarvestTerre', "terre")
	enpelle = false
    DeleteEntity(shovel)
	...
	if pellebroken >= 30 then 
		...
		AeroEvent("sTerreActivity:BrokenPelle","pelle")
		pellebroken = 0
	end
    return true
end
