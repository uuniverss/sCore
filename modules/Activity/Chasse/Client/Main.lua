        ...
        for k,v in pairs(KadirChasse.Position) do
            if not openedMenu then
                if #(pos-v.pedPos) < 1.5 then
                    nofps = true
                    ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour parler au ~c~Gérant des Chasseurs~s~ (permis de chasse requis) !")
...
AddEventHandler("sChasse:returnStart", function(chasse)
    ...
    RageUI.Popup({message = "Bonne ~o~chasse~s~ ! Je te fais confiance, je ne te demande pas ton permis de chasse, je t'ai mis un fusil dans ton sac et un véhicule proche de toi."})
    ...
    if not ESX.Game.IsSpawnPointClear(chasse.spawnVeh, 5.0) then
        ...
        ESX.Game.SpawnVehicle(hash, chasse.spawnVeh, chasse.headingVeh, function(chasseVehicle)
            ...
            AeroEvent("esx_vehiclelock:givekey","no",plate)
        end)
    end
...
AddEventHandler("sChasse:returnStop", function(chasse, message)
    ...
    if message then
        ...
        RageUI.Popup({message = "Bon retour parmis ~g~nous~s~ ! J'ai pris ton fusil inutile de le chercher."})
    else
