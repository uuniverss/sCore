...
local buttonList = {
    Array =  {"~g~Prendre~s~","~r~Supprimer~s~"},
    Index = 1
}
...
AddEventHandler("sCall:TookCall", function(pos)
    ...
    ShowAboveRadarMessage("Votre appel a été enregistré. (~g~"..dist.."m~s~)")
end)
...
AddEventHandler("sCall:SendMessageCall", function(msg, coords, job, source, tel)
    if CheckServiceLspdLssd() or CheckServiceAmbulance() then
        local pPed = PlayerPedId()
        local pPos = GetEntityCoords(pPed)
        local multiplier = math.random(20, 50)
        ...
        if Call.notif then RemoveNotification(Call.notif) end
        Call.notif = ShowAboveRadarMessage("Accepter: ~g~W~s~. Refuser: ~r~X~s~.")
...
local function openCallMenu(job)
    ...
    local mainMenu = RageUI.CreateMenu("", "Liste des appels", nil, nil, "root_cause",banner)
    ...
    while mainMenu do
        ...
        RageUI.IsVisible(mainMenu, true, false, true, function()
            RageUI.Checkbox("~r~Activer les appels téléphoniques", nil, checkbox, { Style = RageUI.CheckboxStyle.Tick }, function(h, s, a, c)
                checkbox = c;
                end, function()
                    ExecuteCommand("911")
                end, function()
            ...
            RageUI.Button("~r~Annuler l'appel en cours", nil, {RightLabel = "→→"}, true, function(h,a,s)
                if s then
                    if Call.blip then 
                        RemoveBlip(Call.blip)
                        Call.blip = nil  
            ...
            RageUI.Button("~r~Vider les appels", nil, {RightLabel = "→→"}, true, function(h,a,s)
                if s then
                    breakThread = true
                    if Call.notif then RemoveNotification(Call.notif) end
                    Call.calls = {}
