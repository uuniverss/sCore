        ...
        RageUI.IsVisible(mainMenu, true, true, true, function()
            RageUI.Button("Consulter le réservoir du véhicule", nil, {}, true, function(h, a, s)
                if s then
                    ...
                    ActualEssence = ShowAboveRadarMessage("Réservoir du véhicule : ~g~"..math.ceil(GetVehicleFuelLevel(vehicle)).."L~s~.")
                end
            end)
            ...
            RageUI.Button("Faire le plein du véhicule", nil, {RightLabel = "Prix : "..FuelServices.defaultPrice*NeedFuel.."~g~$~s~"}, true, function(h, a, s)
                if s then
                    AeroEvent("sFuel:requestToAddFuelFull", GetVehicleFuelLevel(vehicle))
                    Wait(500*NeedFuel)                
                end
            end)
            ...
            RageUI.Button("Acheter "..ChooseLitre.." ~o~litre d'essence~s~", "Prix : "..FuelServices.defaultPrice*ChooseLitre.."~g~$~s~", {}, true, function(h, a, s)
                if s then
                    AeroEvent("sFuel:requestToAddFuel", GetVehicleFuelLevel(vehicle), ChooseLitre)
                    Wait(500*ChooseLitre)
                end
            end)
        end)
...
CreateThread(function()
    ...
    while true do
        ...
        if not onRemplissage and isNearPump and IsPedInAnyVehicle(PlayerPedId(), true) then
            waiting = 1
            ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour accéder à la ~g~station service~s~.")
...
AddEventHandler("sFuel:animatedPlayer", function(fuel, vehicle)
    onRemplissage = true
    local waiting = true
...
AddEventHandler("sFuel:addFuelToVehicle", function(liters)
    ...
    ESX.ShowNotification("Réaprovisionnement en cours (~g~"..math.floor(liters+0.5).."L~s~).")
