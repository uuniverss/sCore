...
AddEventHandler("pGarage:UpdateParkedStatus", function(plt, prc)
    ...
    if prc ~= nil then
        ...
        if xPlayer.getAccount('cash').money >= prc then
            ...
            SendLogs(3066993,"Garage - Rappelle véhicule","**"..GetPlayerName(source).."** vient de payer ***"..prc.."$ ***\n **License** : "..xPlayer.identifier,"https://discord.com/api/webhooks/1151181886283055246/YxTGaIENNC3ZmjK2zvqOpVvWLnNeSdgUT45cqGZ-rJ4f2tBqtVDGLJ7reFEw15aerH6m")
            ...
            TriggerClientEvent("RageUI:Popup", source, {message="~g~Fonds insuffisant"})
        end
    else
        MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE owner = @owner AND parked = 0 AND plate = @plate",
        {['@owner'] = idd, ['@plate'] = plt},
...
AddEventHandler("pGarage:Givecar", function(trgt, label, veh, plt, prps, parked)
    if trgt then
        ...
        TriggerClientEvent("esx:showNotification", xPly.source, "Vous avez reçu un(e) ~g~"..veh.."~s~ ["..plt.."] !")
