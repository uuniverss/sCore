...
RMenu.Add('garage', 'main', RageUI.CreateMenu("", "~g~Garage", 0, 0,"root_cause","garage"))
RMenu:Get('garage', 'main').EnableMouse = false

RMenu.Add('garage', 'submycars', RageUI.CreateSubMenu(RMenu:Get('garage', 'main'), "", "~g~Garage"))
RMenu.Add('garage', 'suboption', RageUI.CreateSubMenu(RMenu:Get('garage', 'submycars'), "", "~g~Garage"))
RMenu.Add('garage', 'subranger', RageUI.CreateSubMenu(RMenu:Get('garage', 'main'), "", "~g~Garage"))
RMenu.Add('garage', 'subplace', RageUI.CreateSubMenu(RMenu:Get('garage', 'suboption'), "", "~g~Garage"))
                ...
                RageUI.IsVisible(RMenu:Get('garage', 'submycars'), true, false, true, function()
                    ...
                    if #VehTab ~= 0 then
                        for i = 1 , #VehTab,1 do
                            ...
                            --if itm.garage_private == 1 then garageLabel = "~r~Garage privé" elseif itm.garage_private == 0 then garageLabel = "~g~Garage publique" end
                                    ...
                                    if veh == nil and not veh and not DoesEntityExist(veh) then
                                        ...
                                        ESX.TriggerServerCallback('pGarage:StockVehicle',function(valid)
                                            if(valid) then
                                                ...
                                                RageUI.Popup({message="~r~Ce véhicule ne t'appartient pas."})
                                            end
                                        end,vehProps)
                                    end
                                end
        ...
        for k,v in pairs(pGaragemenu) do
            ...
            if not Garage.Menu then
                if mPos <= 10.0 then
                    ...
                    if mPos <= 1.5 then
                        ShowHelpNotification("Appuyez sur ~INPUT_PICKUP~ pour intéragir avec le ~g~garage")
