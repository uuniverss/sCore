...
RMenu.Add('faq', 'main', RageUI.CreateMenu("", "~s~Ever ~g~Life", 0, 200,"root_cause","sneakylife"))
        ...
        Citizen.CreateThread(function()
			while Faq.Menu do
                RageUI.IsVisible(RMenu:Get('faq', 'main'), true, false, true, function()
                    RageUI.Separator("FAQ ~s~Ever ~g~Life")
                    RageUI.Button("Envie d'un véhicule ?", "Vous pouvez louer une voiture à mon collègue juste à droite", {}, true, function(_,_,s)
                    end)
                    RageUI.Button("Gagner votre premier salaire", "Pour commencer à te faire de l'argent, diriges-toi au GoPostal ou encore auprès des restaurants ou des entreprises qui recrutent !", {}, true, function(_,_,s)
                    end)
                    RageUI.Separator("discord.gg/eqZtEwehbw")
                end)
				Wait(0)
			end
		end)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsPauseMenuActive() then
            ...
            PushScaleformMovieFunctionParameterString("~g~"..playerName.."~w~ [~g~"..playerId.."~w~] | discord.gg/eqZtEwehbw")
            PushScaleformMovieFunctionParameterBool(true)
            PushScaleformMovieFunctionParameterString("~s~Ever ~g~Life")
            PushScaleformMovieFunctionParameterBool(true)
...
Citizen.CreateThread(function()
	...
	SetDiscordRichPresenceAsset('logo_png_nv')
