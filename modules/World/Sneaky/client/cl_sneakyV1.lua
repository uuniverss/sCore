...
RMenu.Add('faq', 'main', RageUI.CreateMenu("", "~g~ SkyRoyal~s~RP", 0, 200,"root_cause","sneakylife"))
        ...
        Citizen.CreateThread(function()
			while Faq.Menu do
                RageUI.IsVisible(RMenu:Get('faq', 'main'), true, false, true, function()
                    RageUI.Separator("FAQ ~g~ SkyRoyal")
                    RageUI.Button("Envie d'un véhicule ?", "Vous pouvez louer une voiture à mon collègue juste à droite", {}, true, function(_,_,s)
                    end)
                    RageUI.Button("Gagner votre premier salaire", "Pour commencer à te faire de l'argent, diriges-toi au GoPostal ou encore auprès des restaurants qui recrutent.", {}, true, function(_,_,s)
                    end)
                    RageUI.Separator("discord.gg/Wkgsw7wnxd")
                end)
				Wait(0)
			end
		end)
        ...
        for k,v in pairs(Faqmenu) do
            ...
            if not Faq.Menu then
                if mPos <= 10.0 then
                    att = 1
                    if mPos <= 1.5 then
                        ESX.ShowHelpNotification("Appuyez sur ~INPUT_PICKUP~ pour ~g~intéragir")
...
Citizen.CreateThread(function()
	...
	while true do
        ...
        if Ragdoll then
            ...
            DrawTopNotification("Appuyez sur ~INPUT_CONTEXT~ ou ~INPUT_JUMP~ pour ~g~vous relever~w~.")
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsPauseMenuActive() then
            ...
            PushScaleformMovieFunctionParameterString("~g~"..playerName.."~w~ [~g~"..playerId.."~w~] | discord.gg/Wkgsw7wnxd")
            PushScaleformMovieFunctionParameterBool(true)
            PushScaleformMovieFunctionParameterString("~g~SkyRoyal~s~RP")
            PushScaleformMovieFunctionParameterBool(true)
...
Buttons = {
    {index = 0,name = 'Discord',url = 'discord.gg/Wkgsw7wnxd'},
