...
AddEventHandler("sGarage:backVehicle", function(plate, props, garageId)
            ...
            if #sGarageManager.list[garageId].vehicles > v.max then
                for k,v in pairs(sGarageManager.list[garageId].vehicles) do
                    if v.plate == plate then
                        ...
                        return TriggerClientEvent("esx:showNotification", source, "~r~Erreur~s~~n~Il n'y a plus assez de ~g~places~s~ dans le garage !")
                    end
                end
            end
        end
    ...
    print("fin")
    TriggerClientEvent("esx:showNotification", source, "Votre ~g~véhicule~s~ a été rangé avec ~g~succès~s~ !")
