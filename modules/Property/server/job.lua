...
local annonceList = {
    ["open"] = {
        ...
        message = "L'~g~agence immobilière~s~ est actuellement ~g~ouverte~s~ !\nN'hésiter pas à venir.",
        banner = "CHAR_DYNASTY8"
    },
    ["close"] = {
        title = "Agence immobilière",
        subtitle = "~g~Information",
        message = "L'~g~agence immobilière~s~ est actuellement ~r~fermer~s~ !\nRepassez plus tard.",
        banner = "CHAR_DYNASTY8"
    }
}
        ...
        if cbAttribute then
            if tPlayer.getAccount('bank').money >= price then
                ...
                MySQL.Async.execute("UPDATE property SET owner = @owner, ownerInfo = @ownerInfo WHERE id = @propertyId", {
                    ...
                    TriggerClientEvent("esx:showNotification", tPlayer.source, "Vous avez payer la propriété pour "..price.."~g~$~s~ !")
                    TriggerClientEvent("esx:showNotification", xPlayer.source, "La personne à payer la propriété pour "..price.."~g~$~s~ !")
...
AddEventHandler("sProperty:deleteProperty", function(propertyId)
    ...
    MySQL.Async.execute("DELETE FROM property WHERE id = @propertyId", {
        ...
        TriggerClientEvent("esx:showNotification", xPlayer.source, "~g~Information : ~s~~n~Le ~o~"..property.info.Selected.label.."~s~ a été supprimé avec succès !")
        TriggerClientEvent("sProperty:returnProperty", -1, sPropertyManager.list) 
    end)
end)
        ...
        if cbAttribute then
            if tPlayer.getAccount('bank').money >= price then
                ...
                MySQL.Async.execute("UPDATE garage SET owner = @owner, ownerInfo = @ownerInfo WHERE id = @garageId", {
                    ...
                    TriggerClientEvent("esx:showNotification", tPlayer.source, "Vous avez payer le garage pour "..price.."~g~$~s~ !")
                    TriggerClientEvent("esx:showNotification", xPlayer.source, "La personne à payer le garage pour "..price.."~g~$~s~ !")
...
AddEventHandler("sGarage:deleteGarage", function(garageId)
    ...
    MySQL.Async.execute("DELETE FROM garage WHERE id = @garageId", {
        ...
        TriggerClientEvent("esx:showNotification", xPlayer.source, "~g~Information~s~~n~Le ~o~"..garage.info.Selected.label.."~s~ a été supprimé avec succès !")
        TriggerClientEvent("sGarage:returnGarage", -1, sGarageManager.list) 
    end)
end)
