...
Citizen.CreateThread(function()
    ...
    AeroEvent("sProperty:requestList")
    AeroEvent("sProperty:requestavailableProperties")
    AeroEvent("sProperty:requestKeysList")
end)
...
local function openCreateMenu()
    ...
    while mainMenu do
        ...
        RageUI.IsVisible(mainMenu, true, false, false, function()
            RageUI.Button("Créer une ~g~propriété~s~", nil, {RightLabel = "→"}, true, function(h, a, s)
                if s then
        ...
        RageUI.IsVisible(propertyInfo, true, false, false, function()
            if onProperty == nil then
                RageUI.Separator("")
                RageUI.Separator("~g~Chargement...")
                RageUI.Separator("")
            else
                local infos = onProperty.info
                RageUI.Separator("↓ Informations de la ~g~Propriété~s~ n°~o~"..onProperty.propertyId.."~s~ ↓")
                RageUI.Separator("Type: ~o~"..infos.Selected.label)
                RageUI.Separator("Nom: ~g~"..infos.name)
                RageUI.Separator("Prix: "..infos.price.."~g~$~s~")
                if onProperty.street ~= nil then
                    RageUI.Separator("Avenue: ~g~"..onProperty.street)
                end
                ...
                RageUI.Button("Supprimer la propriété", nil, {RightLabel = "→ ~r~Supprimer~s~"}, true, function(h, a, s)
                    if s then 
                        AeroEvent("sProperty:deleteProperty", onProperty.propertyId)
                        RageUI.CloseAll()
                    end
                end)
            end
        ...
        RageUI.IsVisible(createMenuProperty, true, false, false, function()
            if not CreateProperty.Selected.state then
                ...
                for k,v in pairs(ConfigImmo.Batiment.Property) do
                    RageUI.Button(v.label, "Faites '~g~ENTRER~s~' pour sélectionner la propriété !", {}, true, function(h, a, s)
                        if s then
                ...
                RageUI.Separator("~g~Propriété~s~ - Sélectionné (~g~"..CreateProperty.Selected.label.."~s~)")
                ...
                RageUI.Button("Définir l'entrée", nil, { RightLabel = "~g~Définir~s~ →" }, true, function(_, _, s)
                    if s then
                ...
                RageUI.Button("~g~Valider et sauvegarder", nil, {RightLabel = "→"}, true, function(h, a, s)
                    if s then
                        if CreateProperty.Selected.state == false or CreateProperty.Selected.name == "" or CreateProperty.Selected.name == nil or CreateProperty.Selected.label == "" or CreateProperty.Selected.label == nil or CreateProperty.name == "" or CreateProperty.name == nil or CreateProperty.price == 0 or CreateProperty.price == nil or CreateProperty.price == "" or CreateProperty.price < 2500 or CreateProperty.pos == "" or CreateProperty.pos == nil then 
                            ...
                            AeroEvent("sProperty:saveProperty", CreateProperty, street)
                            ESX.ShowNotification("~g~Création de la propriétée en cours...")
                            RageUI.GoBack()    
                        end
                    end
                end)
...
local function openManagement(owner, propertyId)
    ...
    AeroEvent("sProperty:requestKeysAccess", propertyId)
    ...
    while mainMenu do
        ...
        RageUI.IsVisible(mainMenu, true, true, true, function()
            ...
            if owner == true then
                RageUI.Button("Gérer les clés", nil, {RightLabel = "~g~Gérer~s~ →"}, true, function(h, a, s)
                end, keysMenu)
            end

            RageUI.Button("Stockage", nil, {RightLabel = "~g~Voir et Gérer~s~ →"}, true, function(h, a, s)
                if s then
                    RageUI.CloseAll()
                    local items, weapons, blackMoney = {}, {}, 0
                    AeroEvent("sProperty:requestInventory", propertyId)
        ...
        RageUI.IsVisible(keysMenu, true, true, true, function()
            ...
            RageUI.Button("Personne à proximité", nil, {RightLabel = "~g~Attribuer~s~ →"}, true, function(h, a, s)
                if s then
                    local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
                    if closestPlayer ~= -1 and closestDistance <= 3.0 then
                        AeroEvent("sProperty:addKeysAccess", propertyId, "person", GetPlayerName(GetPlayerServerId(closestPlayer)), GetPlayerServerId(closestPlayer))
                    else
            ...
            RageUI.Button("Destitué les clés de personnes", nil, {RightLabel = "~g~Voir~s~ →"}, true, function(h, a, s)
            end, keysMenuCivil)

            RageUI.Button("Attribuer à un des ~g~métiers~s~", nil, {RightLabel = "~g~Voir~s~ →"}, true, function(h, a, s)
            end, keysMenuJob)

            RageUI.Button("Attribuer à un des ~g~groupes~s~", nil, {RightLabel = "~g~Voir~s~ →"}, true, function(h, a, s)
            end, keysMenuGang)
                ...
                if v.type == "person" then
                    if #v ~= 0 then
                        ...
                        RageUI.Button(v.name, nil, {RightLabel = "Enlever la permission~s~ →"}, true, function(h,a,s)
                            if s then
                                AeroEvent("sProperty:removeKeysAccess", propertyId, v.type, v.name, k)
                            end
                        end)
                    end
                end
        ...
        RageUI.IsVisible(keysMenuJob, true, true, true, function()
            ...
            RageUI.Separator("↓ Liste des ~g~métiers~s~ ↓")
                ...
                RageUI.Button(v.label, "Autorisé: "..autorizedLabel, {RightLabel = "~o~Changer la permission~s~ →"}, true, function(h, a, s)
                    if s then
                        if returnkeysAccess[k] == nil then
                            AeroEvent("sProperty:addKeysAccess", propertyId, "jobList", v.label, k)
                        else
                            AeroEvent("sProperty:removeKeysAccess", propertyId, "jobList", v.label, k)
                        end                    
                    end
                end)
        ...
        RageUI.IsVisible(keysMenuGang, true, true, true, function()
            ...
            RageUI.Separator("↓ Liste des ~g~Groupes~s~ ↓")
            ...
            for k,v in pairs(groupList) do
                ...
                RageUI.Button(v.label, "Autorisé: "..autorizedLabel, {RightLabel = "~o~Changer la permission~s~ →"}, true, function(h, a, s)
                    if s then
                        if returnkeysAccess[k] == nil then
                            AeroEvent("sProperty:addKeysAccess", propertyId, "groupList", v.label, k)
                        else
                            AeroEvent("sProperty:removeKeysAccess", propertyId, "groupList", v.label, k)
                        end
                    end
                end)
...
--[[         RageUI.IsVisible(stockMenu, true, true, true, function()
            ...
            RageUI.Button("Déposer", nil, {RightLabel = "~g~→~s~→"}, true, function(h, a, s)
                if s then
                    ESX.PlayerData = ESX.GetPlayerData()
                end
            end, depositStock)
            
            RageUI.Button("Prendre", nil, {RightLabel = "~g~→~s~→"}, true, function(h, a, s)
                if s then
                    AeroEvent("sProperty:requestInventory", propertyId)
                end
            end, withdrawStock)
            ...
            if #ESX.PlayerData.inventory == 0 then            
                ...
                for i = 1, #ESX.PlayerData.accounts, 1 do
                    if ESX.PlayerData.accounts[i].name == 'cash'  then
                        RageUI.Button('Argent liquide : '..ESX.PlayerData.accounts[i].money..'~g~$~s~', nil, {RightLabel = "~g~Déposer~s~ →"}, true, function(h, a, s)
                            if s then
                                local count = tonumber(PropertyInput("Veuillez saisir une ~g~quantité~s~ :", "", 6))
                                if count == nil or count == "" or count < 1 then
                                    ESX.ShowNotification("~r~Erreur~s~~n~Quantité invalide !")
                                else
                                    AeroEvent("sProperty:addItems", propertyId,"money", ESX.PlayerData.accounts[i].name, count, 0)
                                    RageUI.GoBack()
                                end
                            end
                        end)
                    elseif ESX.PlayerData.accounts[i].name == 'dirtycash'  then
                        RageUI.Button('Source inconnue : '..ESX.PlayerData.accounts[i].money..'~c~$~s~', nil, {RightLabel = "~g~Déposer~s~ →"}, true, function(h, a, s)
                            if s then
                                local count = tonumber(PropertyInput("Veuillez saisir une ~g~quantité~s~ :", "", 6))
                                if count == nil or count == "" or count < 1 then
                                    ESX.ShowNotification("~r~Erreur~s~~n~Quantité invalide !")
                                else
                                    AeroEvent("sProperty:addItems", propertyId, "money", "dirtycash", count, 0)
                                    RageUI.GoBack()
                                end
                            end
                        end)
                ...
                RageUI.Separator("↓ ~g~Vos armes~s~ ↓")

                for i = 1, #ESX.PlayerData.loadout do
                    local weapon = ESX.PlayerData.loadout[i]
                    if weapon.ammo == nil then weapon.ammo = 0 end
                    RageUI.Button(weapon.label, weapon.ammo.." Munition(s)", {RightLabel = "~g~Déposer~s~ →"}, true, function(h, a, s)
                        if s then
                            AeroEvent("sProperty:addItems", propertyId, "weapon", weapon.name, 1, weapon.ammo)
                            RageUI.GoBack()
                        end
                    end)
                end
                ...
                for i = 1, #ESX.PlayerData.inventory do
                    ...
                    if item.count > 0 then
                        RageUI.Button(item.label, "Quantité: ~g~"..item.count.."~s~", {RightLabel = "~g~Déposer~s~ →"}, true, function(h, a, s)
                            if s then
                                local count = tonumber(PropertyInput("Veuillez saisir une ~g~quantité~s~ :", "", 6))
                                if count == nil or count == "" or count < 1 then
                                    ESX.ShowNotification("~r~Erreur~s~~n~Quantité invalide !")
                                else
                                    AeroEvent("sProperty:addItems", propertyId, "item", item.name, count, 0)
                                    RageUI.GoBack()
                                end
                            end
                        end)
        ...
        --[[ RageUI.IsVisible(withdrawStock, true, true, true, function()
            if stockInventory == nil then
                ...
                if #stockInventory == 0 then
                    ...
                    RageUI.Separator("↓ ~g~Stockage~s~ ↓")

                    for k,v in pairs(stockInventory) do
                        if v.type == "money" then
                            if v.name == "cash" then
                                RageUI.Button(v.label, v.count.."~g~$~s~", {RightLabel = "~g~Prendre~s~ →"}, true, function(h, a, s)
                                    if s then
                                        local count = tonumber(PropertyInput("Veuillez saisir une ~g~somme~s~ :", "", 6))
                                        if count == nil or count == "" or count < 1 then
                                            ESX.ShowNotification("~r~Erreur~s~~n~Quantité invalide !")
                                        else
                                            AeroEvent("sProperty:removeItems", propertyId, "money", v.name, count, 0)
                                            RageUI.GoBack()
                                        end
                                    end
                                end)
                            elseif v.name == "dirtycash" then
                                RageUI.Button(v.label, v.count.."~c~$~s~", {RightLabel = "~g~Prendre~s~ →"}, true, function(h, a, s)
                                    if s then
                                        local count = tonumber(PropertyInput("Veuillez saisir une ~g~somme~s~ :", "", 6))
                                        if count == nil or count == "" or count < 1 then
                                            ESX.ShowNotification("~r~Erreur~s~~n~Quantité invalide !")
                                        else
                                            AeroEvent("sProperty:removeItems", propertyId, "money", "dirtycash", count, 0)
                                            RageUI.GoBack()
                                        end
                                    end
                                end)
                        ...
                        if v.type == "item" then
                            if v.count > 0 then
                                RageUI.Button(v.label, "Quantité: ~g~"..v.count.."~s~", {RightLabel = "~g~Prendre~s~ →"}, true, function(h, a, s)
                                    if s then
                                        local count = tonumber(PropertyInput("Veuillez saisir une ~g~quantité~s~ :", "", 6))
                                        if count == nil or count == "" or count < 1 then
                                            ESX.ShowNotification("~r~Erreur~s~~n~Quantité invalide !")
                                        else
                                            AeroEvent("sProperty:removeItems", propertyId, "item", v.name, count, 0)
                                            RageUI.GoBack()
                                        end
                                    end
                                end)
                        ...
                        if v.type == "weapon" then
                            if v.count > 0 then
                                RageUI.Button(v.label, "Munition(s): ~o~"..v.ammo.."~s~", {RightLabel = "~g~Prendre~s~ →"}, true, function(h, a, s)
                                    if s then
                                        AeroEvent("sProperty:removeItems", propertyId, "weapon", v.name, 1, v.ammo)
                                        RageUI.GoBack()
                                    end
                                end)
                            end
            ...
            if #(vector3(infos.pos.x, infos.pos.y, infos.pos.z)-myCoords) < 2.0 then
                ...
                if v.ownerLicense ~= "none" then
                    ...
                    if v.ownerLicense == myLicense then
                        ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour rentrer dans ~g~la propriété~s~.")
                        if IsControlJustReleased(0, 54) then
                            AeroEvent("sProperty:enter", v.propertyId, true)
                        end
                    else
                        while playerKeyAcces[v.propertyId] == nil do
                            Wait(50)
                            AeroEvent("sProperty:getKeysAccesPlayer", v.propertyId)
                        end

                        if not playerKeyAcces[v.propertyId] then
                            ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~g~sonner~s~.")
                            if IsControlJustReleased(0, 54) then
                                AeroEvent("sProperty:requestInvitePlayer", v.propertyId, false)
                            end
                        else
                            ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~g~entrer avec vos clés~s~.")
                            if IsControlJustReleased(0, 54) then
                                AeroEvent("sProperty:enter", v.propertyId, "key")
                            end
                        end
                    end
                else
                    ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~g~visiter~s~.")
                    if IsControlJustReleased(0, 54) then
                        AeroEvent("sProperty:enter", v.propertyId, false)
                    end
                end
            end
        end
...
AddEventHandler("sProperty:returnEnter", function(lastpos, table, owner, infosProperty, inventory)
        ...
        while onProperty do
            ...
            for k, v in pairs(PropsInteract) do
                ...
                if object and DoesEntityExist(object) and GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(object), true) < 1.0 and ObjectsDetails[v] and not isInAnim then
                    nofps = true
                    DrawText3D(GetEntityCoords(object).x, GetEntityCoords(object).y, GetEntityCoords(object).z +.5, "Appuyez sur ~g~E ~s~"..ObjectsDetails[v].txt.."~s~.", 15)
            ...
            if #(pCoords-table.interiorEntry.pos) < 1.25 then
                nofps = true
                ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~g~sortir de la propriété~s~.")
                if IsControlJustReleased(0, 54) then
                    onProperty = false
                end
            end
            ...
            if owner == true then
                if #(pCoords-table.managerLocation) < 1.25 then
                    nofps = true
                    ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~g~ouvrir la gestion de la propriété~s~.")
                    if IsControlJustReleased(0, 54) then
    ...
    DoScreenFadeIn(750)
    FreezeEntityPosition(PlayerPedId(), false)
    AeroEvent("sProperty:exitProperty", infosProperty.propertyId)
	...
	Citizen.CreateThread(function()
		...
		while InProgress do
			...
			if IsControlPressed(0, 29) then
				...
				InProgress = false
				count = 0
                AeroEvent("sProperty:returnCbAtribute", request, true)
			elseif IsControlPressed(0, 58) then
				RageUI.Popup({message="~r~Vous avez décliner le paiement"})
				InProgress = false
				count = 0
                AeroEvent("sProperty:returnCbAtribute", request, false)
			end
...
AddEventHandler("sProperty:returnInvitePlayer", function(name, r)
    ...
    RageUI.PopupChar({
		message = "La personne ~g~"..name.."~s~ veut se faire inviter\n\n~g~B~s~ Accepter. | ~r~g~s~ Décliner.",
		picture = "CHAR_CHAT_CALL",
		title = "~g~Propriété~s~",
		iconTypes = 1,
		sender = "Invitation"
	...
	Citizen.CreateThread(function()
		...
		while InProgress do
			...
			if IsControlPressed(0, 29) then
				...
				InProgress = false
				count = 0
                AeroEvent("sProperty:returnCbInvite", r, true)
			elseif IsControlPressed(0, 58) then
				RageUI.Popup({message="~r~Vous avez décliner l'invitation"})
				InProgress = false
				count = 0
                AeroEvent("sProperty:returnCbInvite", r, false)
			end
