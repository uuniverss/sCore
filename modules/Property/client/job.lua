    ...
    while mainMenu do
        ...
        RageUI.IsVisible(mainMenu, true, true, true, function()
            if onService then
                ...
                RageUI.List("Annonce", announce.list, announce.index, "~r~Information~s~ : faites '~g~ENTER~s~' pour passer l'annonce !", {}, true, function(h, a, s, i)
                    announce.index = i
                    if s then
                        announce.type = ""
                        if i == 1 then
                        ...
                        AeroEvent("sProperty:sendAnnounce", announce.type)
                    end
                end)
            ...
            for k,v in pairs(listProperties) do
                ...
                if v.ownerLicense ~= "none" then  
                    RageUI.Button(infos.name, "Modèle : ~o~"..infos.Selected.label.."~s~\nAppartenant à une personne : ~g~Oui~s~\nNom de la personne : "..v.ownerInfo, {RightLabel = "~r~Distituer~s~ →"}, true, function(h, a, s)
                        if s then
                            AeroEvent("sProperty:distitueProperty", v.ownerLicense, v.propertyId)
                        end
                    end)
                else 
                    RageUI.Button(infos.name, "Modèle : ~o~"..infos.Selected.label.."~s~\nAppartenant à une personne : ~r~Non~s~", {RightLabel = "~g~Attribuer~s~ →"}, true, function(h, a, s)
                        if s then
                            ...
                            if closestPlayer ~= -1 and closestDistance <= 3.0 then
                                AeroEvent("sProperty:attributeProperty", GetPlayerServerId(closestPlayer), v.propertyId)
                            else
        ...
        RageUI.IsVisible(gestionGarages, true, true, true, function()
                ...
                for k,v in pairs(listGarages) do
                    ...
                    if v.ownerLicense ~= "none" then  
                        RageUI.Button(infos.name, "Modèle : ~o~"..infos.Selected.label.."~s~\nAppartenant à une personne : ~g~Oui~s~\nNom de la personne : "..v.ownerInfo, {RightLabel = "~r~Distituer~s~ →"}, true, function(h, a, s)
                            if s then
                                AeroEvent("sGarage:distitueGarage", v.ownerLicense, v.garageId)
                            end
                        end)
                    else 
                        RageUI.Button(infos.name, "Modèle : ~o~"..infos.Selected.label.."~s~\nAppartenant à une personne : ~r~Non~s~", {RightLabel = "~g~Attribuer~s~ →"}, true, function(h, a, s)
                            if s then
                                local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
                                if closestPlayer ~= -1 and closestDistance <= 3.0 then
                                    AeroEvent("sGarage:attributeGarage", GetPlayerServerId(closestPlayer), v.garageId)
                                else
                                    ESX.ShowNotification("~r~Erreur~s~~n~Personne à proximiter !")
                                end                                
                            end
    ...
    while true do
        ...
        if job then
            if #(myCoords-ConfigImmo.Pos.Interaction) < 1.5 then
                nofps = true
                ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour gérer ~g~l'entreprise~s~.")
...
function OpenDynasty8ActionMenuRageUIMenu()
    ...
    if Dynasty8ActionMenu.Menu then 
        ...
        RMenu:Get('dynasty8', 'main'):SetSubtitle("~g~Dynasty 8")
