...
Citizen.CreateThread(function()
    ...
    AeroEvent("sGarage:requestList")
    AeroEvent("sGarage:requestAvailableGarages")
end)
    ...
    while mainMenu do
        ...
        RageUI.IsVisible(garageInfo, true, false, false, function()
            if onGarage == nil then
                ...
                RageUI.Separator("↓ Informations du ~o~Garage~s~ n°~g~"..onGarage.garageId.."~s~ ↓")
                ...
                if onGarage.street ~= nil then
                    RageUI.Separator("Avenue: ~g~"..onGarage.street)
                end
                ...
                RageUI.Button("Supprimer le garage", nil, {RightLabel = "→ ~r~Supprimer~s~"}, true, function(h, a, s)
                    if s then 
                        AeroEvent("sGarage:deleteGarage", onGarage.garageId)
                        RageUI.CloseAll()
                    end
                end)
            end
        ...
        RageUI.IsVisible(createMenuGarage, true, false, false, function()
            if not CreateGarage.Selected.state then
                ...
                for k,v in pairs(ConfigImmo.Batiment.Garage) do
                    RageUI.Button(v.label, "Faites '~g~ENTER~s~' pour sélectionner le garage !", {}, true, function(h, a, s)
                        if s then
                ...
                RageUI.Separator("~o~Garage~s~ - Sélectionné (~g~"..CreateGarage.Selected.label.."~s~)")
                ...
                RageUI.Button("~g~Valider et sauvegarder", nil, {RightLabel = "→"}, true, function(h, a, s)
                    if s then
                        if CreateGarage.Selected.state == false or CreateGarage.Selected.name == "" or CreateGarage.Selected.name == nil or CreateGarage.Selected.label == "" or CreateGarage.Selected.label == nil or CreateGarage.name == "" or CreateGarage.name == nil or CreateGarage.price == 0 or CreateGarage.price == nil or CreateGarage.price == "" or CreateGarage.price <= 2500 or CreateGarage.pos == "" or CreateGarage.pos == nil then 
                            ...
                            AeroEvent("sGarage:saveGarage", CreateGarage, street)
...
local function openManagementVehicle(vehicles)
    ...
    while mainMenu do
        ...
        RageUI.IsVisible(mainMenu, true, false, false, function()
            if #vehicles > 0 then
                RageUI.Separator("↓ Liste de vos ~g~véhicules~s~ ↓")
                for k,v in pairs(vehicles) do
                    props = v.props
                    RageUI.Button(GetLabelText(GetDisplayNameFromVehicleModel(props.model)), "Plaque: ~g~"..props.plate, {RightLabel = "~g~Voir plus~s~ →"}, true, function(h, a, s)
                        if s then
                            infoVeh = v
                        end
                    end, infosVehicle)
        ...
        RageUI.IsVisible(infosVehicle, true, false, false, function()
            if infoVeh == nil then
                ...
                props = infoVeh.props
                RageUI.Separator("Modèle du véhicule: ~g~"..GetLabelText(GetDisplayNameFromVehicleModel(props.model)))
                RageUI.Separator("Plaque du véhicule: ~g~"..props.plate)
                RageUI.Separator("Essence du véhicule: "..math.ceil(props.fuelLevel).."~g~/~s~100%")
                RageUI.Separator("État du véhicule: "..math.ceil(props.bodyHealth).."~g~/~s~1000%")
                RageUI.Separator("État du moteur: "..math.ceil(props.engineHealth).."~g~/~s~1000%")
            end
        end)
                ...
                if v.ownerLicense ~= "none" then
                    ...
                    if v.ownerLicense == myLicense then
                        if not IsPedInAnyVehicle(PlayerPedId(), false) then
                            ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour rentrer dans ~g~le garage~s~.")
                            if IsControlJustReleased(0, 54) then
                                AeroEvent("sGarage:enter", v.garageId, true)
                            end
                        else
                            ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour rentrer votre ~g~véhicule~s~ dans le garage.")
                            if IsControlJustReleased(0, 54) then
                                local plate = GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId(), false))
                                AeroEvent("sGarage:backVehicle", plate, ESX.Game.GetVehicleProperties(GetVehiclePedIsIn(PlayerPedId(), false)), v.garageId)                            end
                        end
                    end
                else
                    ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~g~visiter~s~.")
                    if IsControlJustReleased(0, 54) then
                        AeroEvent("sGarage:enter", v.garageId, false)
                    end
                end
            end
        end
...
AddEventHandler("sGarage:returnEnter", function(lastpos, table, owner, infosGarage, vehicles)
    ...
    CreateThread(function()
        ...
        while onGarage do
            ...
            if owner and #(pCoords-table.manager) < 1.25 then
                nofps = true
                ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~g~gérer vos véhicules~s~.")
                if IsControlJustReleased(0, 54) then
            ...
            if #(pCoords-table.entry.pos) < 1.0 then
                nofps = true
                ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~g~sortir du garage~s~.")
                if IsControlJustReleased(0, 54) then
                    stopVeh = false
                    onGarage = false
                end
            ...
            if owner and IsPedInAnyVehicle(PlayerPedId(), false) then
                ...
                for k, v in pairs(vehicles) do
                    ...
                    if plateVerif == plateBonne then
                        AeroEvent("sGarage:outWithVeh", v.plate, v.props, infosGarage.garageId)
                        stopVeh = true
                        onGarage = false
                    end
                end
    ...
    DoScreenFadeIn(750)
    FreezeEntityPosition(PlayerPedId(), false)
    AeroEvent("sGarage:exitGarage", infosGarage.garageId)
	...
	Citizen.CreateThread(function()
		...
		while InProgress do
			...
			if IsControlPressed(0, 29) then
				...
				InProgress = true
				count = 0
                AeroEvent("sGarage:returnCbInvite", r, InProgress)
			elseif IsControlPressed(0, 58) then
...
AddEventHandler("sGarage:outVehicle", function(table, props)
    ...
    ESX.ShowNotification("~g~Information~s~~n~Vous venez de sortir avec votre véhicule ~o~"..GetLabelText(GetDisplayNameFromVehicleModel(props.model)).."~s~ ["..props.plate.."] !")
