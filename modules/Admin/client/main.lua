...
local godmodechecked = false
local invisible = false
local godmode = false
local function starts(String, Start)
    return string.sub(String, 1, string.len(Start)) == Start
end
local ReportsTable, reportCount, take = {},0,0
local Players, connecteds, staff, items = {},0,0, {}
selectedReport = nil
...
AddEventHandler("sAdmin:transmiteinformation", function(index,informations)
    if index >= 2 and index <= 4 then
        ...
        ESX.ShowNotification("Informations : ~g~"..informations.."~s~.")
    end
end)
...
function OpenTestRageUIMenu()
    ...
    if Test.Menu then 
        ...
        RMenu.Add('staff', 'stafflist', RageUI.CreateSubMenu(RMenu:Get("staff", "main"),"", "~g~Menu administration"))
        RMenu.Add('staff', 'playerslist', RageUI.CreateSubMenu(RMenu:Get("staff", "main"),"", "~g~Menu administration"))
        RMenu.Add('staff', 'playersaction', RageUI.CreateSubMenu(RMenu:Get("staff", "playerslist"),"", "~g~Menu administration"))
        RMenu.Add('staff', 'optionsplayer', RageUI.CreateSubMenu(RMenu:Get("staff", "playersaction"),"", "~g~Menu administration"))
        RMenu.Add('staff', 'joblist', RageUI.CreateSubMenu(RMenu:Get("staff", "optionsplayer"),"", "~g~Menu administration"))
        RMenu.Add('staff', 'grouplist', RageUI.CreateSubMenu(RMenu:Get("staff", "optionsplayer"),"", "~g~Menu administration"))
        RMenu.Add('staff', 'remboursement', RageUI.CreateSubMenu(RMenu:Get("staff", "optionsplayer"),"", "~g~Menu administration"))
        RMenu.Add('staff', 'itemlist', RageUI.CreateSubMenu(RMenu:Get("staff", "remboursement"),"", "~g~Menu administration"))
        RMenu.Add('staff', 'warnlist', RageUI.CreateSubMenu(RMenu:Get("staff", "playersaction"),"", "~g~Menu administration"))
        RMenu.Add('staff', 'banlist', RageUI.CreateSubMenu(RMenu:Get("staff", "playersaction"),"", "~g~Menu administration"))
        RMenu.Add('staff', 'vehiclemain', RageUI.CreateSubMenu(RMenu:Get("staff", "main"),"", "~g~Menu administration"))
        RMenu.Add('staff', 'diversmain', RageUI.CreateSubMenu(RMenu:Get("staff", "main"),"", "~g~Menu administration"))
        RMenu.Add('staff', 'reportsmain', RageUI.CreateSubMenu(RMenu:Get("staff", "main"),"", "~g~Menu administration"))
        RMenu.Add('staff', 'reportsaction', RageUI.CreateSubMenu(RMenu:Get("staff", "reportsmain"),"", "~g~Menu administration"))
        RMenu:Get('staff', 'main'):SetSubtitle("~g~Menu administration")
        RMenu:Get('staff', 'main').EnableMouse = false
        RMenu:Get('staff', 'main').Closed = function()
            filterstringstaff = ""
            Test.Menu = false
            AeroEvent("SneakyLife:StaffState", false)
        end
        Test.Menu = true
        AeroEvent("SneakyLife:StaffState", true) 
			...
			while Test.Menu do
                ...
                RageUI.IsVisible(RMenu:Get('staff', 'reportsmain'), true, false, true, function()
                    ...
                    if not NamesRp then
                        for sender, infos in pairs(localReportsTable) do
                            if infos.taken then
                                if hideTakenReports == false then
                                    RageUI.Button(getIsTakenDisplay(infos.taken).." "..infos.id .." - "..infos.name,"~g~Pris en charge par :~s~ "..infos.takenBy.."~n~~g~Motif :~s~ "..infos.reason.."~n~~g~Date : ~s~"..infos.timeElapsed[2].."h"..infos.timeElapsed[1].."m", { RightLabel = "→" }, true, function(_, _, s)
                                        if s then
                                ...
                                RageUI.Button(getIsTakenDisplay(infos.taken).." "..infos.id.." - "..infos.name,"~g~Pris en charge par :~s~ Personne~n~~g~Motif :~s~ "..infos.reason.."~n~~g~Date : ~s~"..infos.timeElapsed[2].."h"..infos.timeElapsed[1].."m", { RightLabel = "→" }, true, function(_, _, s)
                                    if s then
                                        selectedReport = sender
                                    end
                                end,RMenu:Get("staff","reportsaction"))
                        ...
                        for sender, infos in pairs(localReportsTable) do
                            ...
                            if infos.taken then
                                if hideTakenReports == false then
                                    RageUI.Button(getIsTakenDisplay(infos.taken).." "..infos.id .." - "..identityrp,"~g~Pris en charge par :~s~ "..infos.takenBy.."~n~~g~Motif :~s~ "..infos.reason.."~n~~g~Date : ~s~"..infos.timeElapsed[2].."h"..infos.timeElapsed[1].."m", { RightLabel = "→" }, true, function(_, _, s)
                                        if s then
                                            selectedReport = sender
                                        end
                                    end,RMenu:Get("staff","reportsaction"))
                                ...
                                RageUI.Button(getIsTakenDisplay(infos.taken).." "..infos.id.." - "..identityrp,"~g~Pris en charge par :~s~ Personne~n~~g~Motif :~s~ "..infos.reason.."~n~~g~Date : ~s~"..infos.timeElapsed[2].."h"..infos.timeElapsed[1].."m", { RightLabel = "→" }, true, function(_, _, s)
                                    if s then
                                        selectedReport = sender
                                    end
                                end,RMenu:Get("staff","reportsaction"))
                    ...
                    if localReportsTable[selectedReport] ~= nil then
                        ...
                        if not localReportsTable[selectedReport].taken then
                            RageUI.Button("Prendre le ticket", nil, {}, true, function(_, _, s)
                                if s then
                                    AeroEvent("SneakyLife:takeReport", selectedReport)
                        ...
                        RageUI.Button("Clôturer le ticket", nil, {}, true, function(_, _, s)
                            if s then
                                AeroEvent("SneakyLife:CloseReport", selectedReport)
                            end
                        end)
                    else
                        RageUI.Separator("")
                ...
                RageUI.IsVisible(RMenu:Get('staff', 'playerslist'), true, false, true, function()
                    if IsControlJustPressed(0, 22) then
                        filterstringstaff = Rechercher("entysearch", "~g~Rechercher", "", 50)
                    end
                    RageUI.Separator("Joueur(s) en ligne ~g~ " ..connecteds)
                ...
                RageUI.IsVisible(RMenu:Get('staff', 'playersaction'), true, false, true, function()
                    ...
                    RageUI.List("Informations", information, informationindex, nil, {}, true, function(Hovered, Active, Selected, Index) 
                        if (Selected) then 
                            ...
                            AeroEvent('Sneakygetinformation', idjoueur, Index)
                        end 
                    ...
                    RageUI.Button("Avertissement", nil, { RightLabel = ""}, true, function(h,a,s)
                        if s then
                            ...
                            if Reason == "" or Reason == nil then
                                ...
                                local idjoueur = IdSelected
                                ESX.ShowNotification("~g~ Ever Life~s~\nVous venez de warn ~c~"..GetPlayerName(GetPlayerFromServerId(idjoueur)).."~s~ avec comme raison : ~g~"..Reason)
                    ...
                    RageUI.Button("Kick", nil, {RightLabel = ""}, true, function(Hovered, Active, Selected)
                        if (Selected) then
                            local reason = KeyboardInputAdmin("~g~Raison :", "", 100)
                            idjoueur = IdSelected
                    ...
                    RageUI.Button("Voir l'inventaire", nil, {RightLabel = ""}, true,function(h,a,s)
                        if s then
                            RageUI.CloseAll()
                            AeroEvent("sStaff:OpenPlayerInventory",IdSelected)
                        end
                    end)
...
--]]
                        ...
                        if starts(itemInfos.label:lower(), filterArray[filter]:lower()) then
                            RageUI.Button(itemInfos.label.." - ~c~"..itemInfos.name, nil, { RightLabel = "→" }, true, function(_, _, s)
                                if s then
                                    ...
                                    local amount = CustomAmount2()
                                    if amount ~= nil and amount > 0 then
                                        AeroEvent("Sneaky:GiveItem", idjoueur, itemInfos.name, amount)
                                    end
                                end
                            end)
                        end
                ...
                RageUI.IsVisible(RMenu:Get('staff', 'warnlist'), true, false, true, function()
                    ...
                    for i = 1, #wList, 1 do
                        local v = wList[i]
                        RageUI.Button("Raison : ~g~"..v.reason,"Par : ~g~".. v.moderator.."~s~ le "..v.date,{RightLabel = "~r~Supprimer~s~ →"},true,function(_, _, Selected)
                            if Selected then
                                if (ESX.GetPlayerData()['group']) == "_dev" then
                ...
                RageUI.IsVisible(RMenu:Get('staff', 'banlist'), true, false, true, function()
                    ...
                    for i = 1, #bList, 1 do
                        local v = bList[i]
                        RageUI.Button("Raison : ~g~"..v.reason,"Par : ~g~".. v.moderator.."~s~ le "..v.date,{RightLabel = ""},true,function(_, _, Selected)
                        end)
                    end
                end)
                RageUI.IsVisible(RMenu:Get('staff', 'joblist'), true, false, true, function()
                    RageUI.Button("~g~Filtrer",nil,{RightLabel = "🔍"},true,function(_, _, Selected)
                        if Selected then
                        ...
                        if filterString == nil or string.find(v.name, filterString) or string.find(_, filterString) then
                            for k,i in pairs(v.grade) do
                                RageUI.Button(v.name.." - ~c~["..k.."] "..i.label, nil, {RightLabel = ""}, true,function(h,a,s)
                                    if s then
                                        idjoueur = IdSelected
                                        ESX.ShowNotification("Vous avez séléctionné le job ~g~"..v.name.." ~s~avec le grade ~c~"..k)
                        ...
                        if filterString == nil or string.find(v.name, filterString) or string.find(_, filterString) then
                            for k,i in pairs(v.grade) do
                                RageUI.Button(v.name.." - ~c~["..k.."] "..i.label, nil, {RightLabel = ""}, true,function(h,a,s)
                                    if s then
                                        idjoueur = IdSelected
                                        ESX.ShowNotification("Vous avez séléctionné le groupe ~g~"..v.name.." ~s~avec le grade ~c~"..k)
                        ...
                        RageUI.Button("Créer un véhicule", nil, {}, true, function(h,a,s)
                            if s then
                                ...
                                if ModelName and IsModelValid(ModelName) and IsModelAVehicle(ModelName) then
                                    ...
                                    ESX.ShowNotification("~g~Menu staff\n~s~Le nom du véhicule est incorrect ~g~¦")
                                end
                            end
                        end)
                ...
                RageUI.IsVisible(RMenu:Get('staff', 'diversmain'), true, false, true, function()
                    ...
                    if (ESX.GetPlayerData()['group']) == "_dev" or (ESX.GetPlayerData()["group"]) == "gs" then
                    RageUI.Checkbox("Godmode", false, godmodechecked,{Style = RageUI.CheckboxStyle.Tick}, function(Hovered,Ative,Selected,Checked)    
                        godmodechecked = Checked
                        if Checked then
                            if Selected then
                                godmode = true
...
--                              ESX.ShowNotification('Godmode ~r~désactivé')
                    ...
                    RageUI.Checkbox("Invisible",nil, invisiblechecked, {Style = RageUI.CheckboxStyle.Tick}, function(Hovered,Active,Selected,Checked)
                        ...
                        if s then 
                            ...
                            if DoesBlipExist(waypointHandle) then
                                Citizen.CreateThread(function()
                                    ...
                                    ESX.ShowNotification('~g~<C>TP~s~ Effectuer<C>~s~.')
                                end)
                            else
                                ESX.ShowNotification('Aucun ~g~<C>marqueur !~s~<C>')
                            end
                        end
                    end)
...
Citizen.CreateThread(function()
    while true do
        if invisible then
            ...
            Wait(0)
            DrawMissionText("~g~Invisible",500)
        else
...
Citizen.CreateThread(function(force, wasFall)
    while true do
        if godmode then
...
Keys.Register('F10','F10', 'Menu Administrateur ', function()
    if not exports.inventaire:GetStateInventory() and not GetStateFishing() then
        if (ESX.GetPlayerData()['group'] == "_dev") or (ESX.GetPlayerData()['group']) == "gs" or (ESX.GetPlayerData()['group'] == "superadmin") or (ESX.GetPlayerData()['group'] == "admin") then
            AeroEvent("sAdmin:CheckGroup",ESX.GetPlayerData()['group']) 
            OpenTestRageUIMenu()
        end
    end
end)
...
AddEventHandler("core:envoyer", function(namestaff, msg)
	...
	ESX.ShowAdvancedNotification('Message Staff', '~g~Informations~s~ (~c~<C>'..namestaff..'~s~<C>)', msg, "CHAR_SNEAKY", 1)
end)
...
AddEventHandler("SneakyAdmin:RequestCoordsCamToServer", function()
    if GetSpecateBoolStaff() then
        ...
        AeroEvent("SneakyLife:RequestCoordsCam",CamCoords)
    end
end)
...
function Admin:Spectate(pPos)
    ...
    if not Admin.InSpec then
        AeroEvent("SneakyLife:ChangeStateSpectate",false)
        ...
        invisible = true
        Admin:CreateCam()
        AeroEvent("SneakyLife:ChangeStateSpectate",true)
